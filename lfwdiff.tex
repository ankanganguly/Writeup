...
%DIF ADD local_finite_writeup.tex       Mon Dec 10 18:38:25 2018
...
\tr{TODO: rigorously solve this exercise myself}}\DIFdelend \DIFaddbegin \DIFadd{See Lemma \ref{sec::TL:Lem::embedding}}\DIFaddend ).
...
Since this holds for every rectangle, \DIFdelbegin \DIFdel{\(\poiss\sln{n}\) }\DIFdelend \DIFaddbegin \DIFadd{and since the rectangles form a dissecting ring on \([0,\T]\times \mspce \times [0,\const]\) for every \(\T,\const < \infty\), \(\alt{\poiss}\) }\DIFaddend is a Poisson process on \DIFdelbegin \DIFdel{\([0,T]\times \alt{\S}\carp{n}\times [0,\const]\) by R\'enyi, M\"onch \mbox{%DIFAUXCMD
\cite[Theorem 9.2.XII]{DalVer08}}\hspace{0pt}%DIFAUXCMD
.
