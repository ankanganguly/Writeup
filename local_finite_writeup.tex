...
\newcommand{\sv}[1]{\s^{#1}}					%typ elt of \XState
\renewcommand{\b}[1]{b_{#1}}					%inf dim norm coeffs
\newcommand{\ev}[1]{\ep_{#1}}					%standard basis
\newcommand{\T}{T}								%time bound
\newcommand{\OmegaV}[2]{\Omega_{#1}^{#2}}		%Path space
\newcommand{\x}{x}								%element of OmegaV
...
\newcommand{\xvt}[2]{x_{#1}{(#2)}}					%typ path val
\newcommand{\xvtt}[2]{y_{#1}{(#2)}}					%typ path val2
\newcommand{\xvttt}[2]{z_{#1}{(#2)}}				%typ path val3
\newcommand{\rxvt}[2]{X_{#1}{(#2)}}					%typ rand path val
\newcommand{\rxvtn}[3]{X_{#1}^{#3}(#2)}				%typ rp val sublim
...
\newcommand{\xvts}[2]{x_{#1}{#2}}					%typ path
\newcommand{\xvtts}[2]{y_{#1}{#2}}					%typ path 2
\newcommand{\xvttts}[2]{z_{#1}{#2}}					%typ path 3
\newcommand{\rxvts}[2]{X_{#1}{#2}}					%typ rand path
\newcommand{\rxvtsn}[3]{X_{#1}^{#3}{#2}}			%typ rpath sublim
...
\newcommand{\F}[2]{\mc{F}_{#1}^{#2}}				%Filtrations
\newcommand{\FF}[2]{\mc{G}_{#1}^{#2}}				%Filtrations
\newcommand{\FFF}[2]{\mc{H}_{#1}^{#2}}				%Filtrations
...
\begin{align*}
\mb{E}\bigg[&\|F_\poisses(\rxvts{}{})_\v - F_\poisses(\alt{\rxvts{}{}})_\v\|_\t\bigg] - \ex{|\rxvt{\v}{0} - \alt{\rxvt{\v}{0}}|}\\
&= \ex{\sup_{\ttt \leq \t}\left|\int_\S\int_0^{\ttt} \s\poiss{\v}\left(d\tt,\left(0,\rate{\v}(\rxvt{}{\tt-},\s)\right],d\s\right) -  \int_\S\int_0^{\ttt} \s\poiss{\v}\left(d\tt,\left(0,\rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right],d\s\right)\right|}\\
&=\ex{\sup_{\ttt\leq \t} \left|\int_\S \int_0^{\const{}} \int_0^{\ttt} \s\left(\mb{I}_{\r < \rate{\v}(\rxvt{}{\tt-},\s)} - \mb{I}_{\r < \rate{\v}(\alt{\rxvt{}{\tt-}},\s)}\right)\,\poiss{\v}(d\tt\,d\r\,d\s)\right|}\\
&\leq \ex{\int_\S\int_0^{\const{}}\int_0^\t \left|\s\left(\mb{I}_{\r < \rate{\v}(\rxvt{}{\tt-},\s)} - \mb{I}_{\r < \rate{\v}(\alt{\rxvt{}{\tt-}},\s)}\right)\right|\,\poiss{\v}(d\tt\,d\r\,d\s)}\\
&=\int_\S\int_0^\t\int_0^{\const{}} |\s|\ex{\mb{I}_{\rate{\v}(\rxvt{}{\tt-},\s) \leq \r < \rate{\v}(\alt{\rxvt{}{\tt-}},\s)} + \mb{I}_{\rate{\v}(\alt{\rxvt{}{\tt-}},\s) \leq \r < \rate{\v}(\rxvt{}{\tt-},\s)}}\,d\r\,d\tt\,\Sm(d\s)\\
&= \int_\S\int_0^\t|\s|\ex{\left|\rate{\v}(\rxvt{}{\tt-},\s) - \rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right|}\,d\tt\,\Sm(d\s)\\
&\leq \const{}\int_\S\int_0^\t |\s| \ex{\sum_{\vv \in \cl{\v}}|\rxvt{\vv}{\tt-} - \alt{\rxvt{\vv}{\tt-}}|}\,d\tt\,\Sm(d\s)\\
&\leq \const{}|\S|\int_0^\t \sum_{\vv \in \cl{\v}}\ex{\|\rxvts{\vv}{} - \alt{\rxvts{\vv}{}}\|_\tt}\,d\tt\\
\end{align*}
...
\begin{lem}
Given the conditions and notation of Lemma \ref{subsec::Well-:Lem::SDEWP}, assume that \(\rxvt{}{0} = \alt{\rxvt{}{0}}\) almost surely. Then,

\[\sup_{\v \in \V} \ex{\left\|F_\poisses(\rxvts{}{})_\v - F_\poisses(\alt{\rxvts{}{}})_\v\right\|_\t} \leq (\degr+1)\const{1}\int_0^\t \sup_{\v \in \V} \ex{\|\rxvts{\v}{} - \alt{\rxvts{\v}{}}\|_\tt}\,d\tt.\]
...
\begin{align*}
\sup_{\v \in \V} \ex{\left\|F_\poisses(\rxvts{}{})_\v - F_\poisses(\alt{\rxvts{}{}})_\v\right\|_\t} &\leq \sup_{\v\in \V}\left(\ex{|\rxvt{\v}{0} - \alt{\rxvt{\v}{0}}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\rxvts{\vv} - \alt{\rxvts{\vv}{}}\|_\tt}\,d\tt\right)\\
&=\sup_{\v \in \V} \const{1}\int_0^\t \sum_{\vv \in \cl{\v}} \ex{\|\rxvts{\vv} - \alt{\rxvts{\vv}{}}\|_\tt}\,d\tt\\
&\leq \const{1}\int_0^\t \sup_{\v \in \V}\sum_{\vv \in \cl{\v}} \ex{\|\rxvts{\vv} - \alt{\rxvts{\vv}{}}\|_\tt}\,d\tt\\
&\leq \const{1}\int_0^\t \sup_{\v \in \V}\sum_{\vv \in \cl{\v}}\sup_{\vvv\in\V} \ex{\|\rxvts{\vvv} - \alt{\rxvts{\vvv}{}}\|_\tt}\,d\tt\\
&= \const{1}\int_0^\t \sup_{\v \in \V}(\degr+1)\sup_{\vvv\in\V} \ex{\|\rxvts{\vvv} - \alt{\rxvts{\vvv}{}}\|_\tt}\,d\tt\\
&= (\degr+1)\const{1}\int_0^\t \sup_{\v\in\V} \ex{\|\rxvts{\v} - \alt{\rxvts{\v}{}}\|_\tt}\,d\tt\\
\end{align*}
\end{proof}
\lin
...
\begin{align}
\rxvtt{\vv}{\t} &= \rxvt{\vv}{\rt{\itt}} + \int_\S\int_0^\t \s\alt{\poiss{\vv}}\left(d\tt,\left(0,\rate{\vv}(\rxvtt{}{\tt-},\s)\right],d\s\right)\te{ for } \vv \in \cl{\U} \te{ on } \rt{\itt} < \infty \label{sec::Proof1:eqn::alternate SDE}\\
\rxvtts{\vv}{} &\equiv 0\te{ for } \vv\notin \cl{\U}\nonumber\\
\alt{\poiss{\vv}}(d\tt,d\r,d\s) &= \poiss{\vv}(\rt{\itt} + d\tt,d\r,d\s) \te{ for all }\vv\in \cl{\U}\nonumber\\
\alt{\rxvt{\vv}{\t}} &= \begin{cases}
\rxvt{\vv}{\t} &\te{ on } \t \leq \rt{\itt}\\
...
\ind \tr{Originally I used the strong Markov property to reduce to the case of \(\itt = 0\). However, since \(\rt{\itt}\) is not an \(\rxvts{}{}\)-stopping time, I need to show that \(\rxvts{}{}\) is strong-Markov or Feller with respect to the larger filtration containing the driving Poisson processes. While this is not that difficult, the argument is tedious and long, so I chose not to use this reduction.}

\ind By Proposition \ref{subsec::Well-:Prop::SDEWP}, equation \eqref{sec::Proof1:eqn::alternate SDE} has a unique solution, so \(\alt{\rxvts{}{}}\) is well-defined. To prove this lemma, we first show that \(\rxvts{\U}{[0,\rt{\itt+1})} = \alt{\rxvts{\U}{}}{[0,\rt{\itt+1})}\) almost surely. 
...
\begin{align*}
&\ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_{\t}\mb{I}_{\t < \rt{\itt+1}}} = \ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_{\t}\mb{I}_{\evnt_\t}}\os{\alt{\rxvts{\cl{\U}}{}}[0,\rt{\itt}] = \rxvts{\cl{\U}}{[0,\rt{\itt}]}}{=} \ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_{\t}\mb{I}_{\alt{\evnt}_\t}}\\
&\ind\leq \ex{\mb{I}_{\alt{\evnt}_\t}\left|\int_\S\int_{(\rt{\itt},t]} \s\,\poiss{\v}\left(d\tt,\left(0,\rate{\v}(\rxvt{}{\tt-},\s)\right]\,d\s\right) - \int_\S\int_{(\rt{\itt},\t]} \s\,\alt{\poiss{\v}}\left(d\tt,\left(0,\rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right]\,d\s\right)\right|}\\
&\ind \leq \ex{\int_\S\int_{\mb{R}}\int_{(\rt{\itt},\t]}|\s|\mb{I}_{\alt{\evnt}_\t}\mb{I}_{\r\in \left(\rate{\v}(\rxvt{}{\tt-},\s), \rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right]}\,\poiss{\v}\left(d\tt,d\r,d\s\right)}\\
&\ind \os{\alt{\rxvts{\cl{\U}}{}}[0,\rt{\itt}] = \rxvts{\cl{\U}}{[0,\rt{\itt}]}}{=} \ex{\int_\S\int_{\mb{R}}\int_{(0,\t]}|\s|\mb{I}_{\evnt_\t}\mb{I}_{\r\in \left(\rate{\v}(\rxvt{}{\tt-},\s), \rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right]}\,\poiss{\v}\left(d\tt,d\r,d\s\right)}\\
&\ind = \int_\S\int_\mb{R}\int_0^\t |\s|\ex{\mb{I}_{\evnt_\t}\mb{I}_{\r\in\left(\rate{\v}(\rxvt{}{\tt-},\s),\rate{\v}(\alt{\rxvt{}{\tt-}},\s)\right]}}\,d\tt\,d\r\,\Sm(d\s)\\
&\ind = \int_\S |\s|\int_0^\t \ex{\mb{I}_{\evnt_t}\left|\rate{\v}(\alt{\rxvt{}{\tt-}},\s) - \rate{\v}(\rxvt{}{\tt-},\s)\right|}\,d\tt\,\Sm(d\s)\\
&\ind \leq \const{}\int_\S|\s|\int_0^\t\ex{\mb{I}_{\evnt_t}\sum_{\vv\in \cl{\v}}\left|\alt{\rxvt{\vv}{\tt-}} - \rxvt{\vv}{\tt-}\right|}\,d\tt\\
&\ind \os{\mb{I}_{\evnt_\t}\|\alt{\rxvts{\neigh{\U}}{}} - \rxvts{\neigh{\U}}{}\|_t = 0}{\leq} \left(\int_\S|\s|\,\Sm(d\s)\right)\const{} \int_0^\t \sum_{\vv\in \cl{\v}\cap\U}\ex{\mb{I}_{\evnt_t}\|\alt{\rxvts{\vv}{}} - \rxvts{\vv}{}\|_\tt}\,d\tt\\
&\ind \os{\evnt_\tt \supseteq \evnt_\t}{\leq} |S|\const{}(\degr+1) \int_0^\t \sup_{\v\in \U}\ex{\mb{I}_{\evnt_\tt}\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\|_\tt}\,d\tt.
\end{align*}

We can conclude that

\[\sup_{\v\in \U}\ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_{\t}\mb{I}_{t <\rt{\itt+1}}} \leq |\S|\const{}(\degr+1)\int_0^\t\sup_{v\in\U}\ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_\tt\mb{I}_{\tt < \rt{\itt+1}}}\,d\tt.\]

By Gronwall's inequality,

\[\sup_{v\in \U}\ex{\left\|\alt{\rxvts{\v}{}} - \rxvts{\v}{}\right\|_\t\mb{I}_{\t < \rt{\itt+1}}} = 0 \te{ for all } \t > 0,\]

so \(\alt{\rxvts{\U}{}}[0,\rt{\itt+1}) = \rxvts{\U}{[0,\rt{\itt+1})}\) almost surely. However, \(\alt{\rxvts{\U}{}}[0,\rt{\itt+1})\) is uniquely defined by \(\rxvts{\cl{\U}}{[0,\rt{\itt}]}\) and \(\poiss{\cl{\U}}\left((\rt{\itt},\rt{\itt+1}]\times \mb{R}\times \S\right)\).
\end{proof}
...
\begin{equation}
\rxvtn{\v}{\t}{\numb} = \begin{cases}
\rxvtn{\v}{0}{} + \int_{(0,\t]}\int_\S\int_{(0,\crate{}{}(\rxvtsn{\{\v,\p{\v}\}}{[0,\tt)}{\numb},\s)]}\s\alt{\poiss{\v}}(d\tt,d\s,d\r)\te{ for } \v \in \tree\pup{\numb}\setminus\tree\pup{\numb-1}\\
\rxvtn{\v}{0}{} + \int_{(0,\t]}\int_\S\int_{(0,\rate{}(\rxvtn{\cl{\v}}{\tt-}{\numb},\s)]}\s\,\poiss{\v}(d\tt,d\s,d\r)\te{ for } \v \in \tree\pup{\numb-1}.
\end{cases}
\label{subsec::ProofU:eqn::ExtendedSDE_fixedPoisson}
\end{equation}
\labe{subsec::ProofU:eqn::ExtendedSDE\_fixedPoisson}

By Lemma \ref{subsec::ProofU:Lem::Poisson}, these Poisson processes are i.i.d. By Lemma \ref{subsec::ProofU:Lem::extendedSDE}, \(\te{law}(\rxvtsn{}{}{\numb}) = \mmm{}{}{\numb}\). By construction, \(\piV{\tree\pup{\numb}}{\T-}(\rxvts{}{}) = \rxvtsn{}{}{\numb}\) for all \(\numb\in\mb{N}\).

\ind Fix any \(\v\in\V\). Let \(\numb\in \mb{N}\) be any natural number such that \(\v\in \tree\pup{\numb-1}\). Then for any \(\t \in [0,\T)\),

\begin{align*}
\rxvt{\v}{0} + \int_{(0,\t]}\int_\S\int_{(0,\rate{}(\rxvt{\cl{\v}}{\tt-},\s)]}\s\poiss{\v}(d\r,d\s,d\tt) &= \rxvt{\v}{0} + \int_{(0,\t]}\int_\S\int_{(0,\rate{}(\rxvtn{\cl{\v}}{\tt-}{\numb},\s)]}\s\poiss{\v}(d\r,d\s,d\tt)\\
&=\rxvtn{\v}{\t}{\numb} = \rxvt{\v}{\t}.
\end{align*}

Thus, \(\rxvts{}{}\) is a solution to the coupled equation,

\begin{equation}
\rxvt{\v}{\t} = \rxvt{\v}{0} + \int_{(0,\t]}\int_\S \int_{(0,\rate{}(\rxvts{}{\tt-},\s)]}\s\,\poiss{\v}(d\r,d\s,d\tt).
