...
\[\cmodu\pra{\X\cind{}\tip{}}(\delta) = \inf_{\{\t{\it}\}} \sup_\it \sup_{\tt,\ttt \in [\t{\it-1},\t{\it})} \|\X\cind{}\tp{\tt} - \X\cind{}\tp{\ttt}\|_{\b},\]
...
\begin{align*}
\pr\left(\sup_n \cmodu\pra{\X\sln{n}\cind{}\tip{}}(\delta) > \epsilon\right) &= \pr\left(\sup_n\inf_{\{\t{\it}\}}\sup_\it\sup_{\tt,\ttt \in [\t{\it-1},\t{\it})} \sum_{\v \in V} \b\cind{\v}|(\X\sln{n}\cind{\v}\tp{\tt} - \X\sln{n}\cind{\v}\tp{\ttt}| > \epsilon \right)\\
...
\[\cmodu\pra{\X}(\delta) = \inf_{\{	{i}\}: \min_i |	{i} - \t{i-1}| > \delta} \max_{i} \modu{\X}([\t{i-1},	{i}))\]
