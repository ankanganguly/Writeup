...
\[\ex{\|\Fpo(\X\cind{}\tip{})\cind{\v} - \Fpo(\alt{\X}\cind{}\tip{})\cind{\v}\|_\t} \leq \ex{|\X\cind{\v}\tp{0} - \alt{\X}\cind{\v}\tp{0}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\]
...
&=\ex{\sup_{\ttt\leq \t} \left|\int_\S \int_0^{\const{}} \int_0^{\ttt} \s\left(\mb{I}_{\r < \IGr\vind{\v}(\X\cind{}\tp{\tt-},\s)} - \mb{I}_{\r < \IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)}\right)\,\poiss\vind{\v}(d\tt\,d\r\,d\s)\right|}\\
&\leq \ex{\int_\S\int_0^{\const{}}\int_0^\t \left|\s\left(\mb{I}_{\r < \IGr\vind{\v}(\X\cind{}\tp{\tt-},\s)} - \mb{I}_{\r < \IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)}\right)\right|\,\poiss\vind{\v}(d\tt\,d\r\,d\s)}\\
&=\int_\S\int_0^\t\int_0^{\const{}} |\s|\ex{\mb{I}_{\IGr\vind{\v}(\X\cind{}\tp{\tt-},\s) \leq \r < \IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)} + \mb{I}_{\IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s) \leq \r < \IGr\vind{\v}(\X\cind{}\tp{\tt-},\s)}}\,d\r\,d\tt\,\Sm(d\s)\\
...
&\leq \const{}|\S|\int_0^\t \sum_{\vv \in \cl{\v}}\ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
\end{align*}

Then,

\[\ex{\|\Fpo(\X\cind{}\tip{})\cind{\v} - \Fpo(\alt{\X})\cind{\v}\|_\t} \leq \ex{|\X\cind{\v}\tp{0} - \alt{\X}\cind{\v}\tp{0}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt.\]
...
\[\sup_{\v \in V} \ex{\left\|\Fpo(\X\cind{}\tip{})\cind{\v} - \Fpo(\alt{\X}\cind{}\tip{})\cind{\v}\right\|_\t} \leq (\degr+1)\const{1}\int_0^\t \sup_{\v \in V} \ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\tt}\,d\tt.\]
...
\begin{align*}
\sup_{\v \in V} \ex{\left\|\Fpo(\X\cind{}\tip{})\cind{\v} - \Fpo(\alt{\X}\cind{}\tip{})\cind{\v}\right\|_\t} &\leq \sup_{\v\in V}\left(\ex{|\X\cind{\v}\tp{0} - \alt{\X}\cind{\v}\tp{0}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\right)\\
&=\sup_{\v \in V} \const{1}\int_0^\t \sum_{\vv \in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&\leq \const{1}\int_0^\t \sup_{\v \in V}\sum_{\vv \in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&\leq \const{1}\int_0^\t \sup_{\v \in V}\sum_{\vv \in \cl{\v}}\sup_{\vvv\in V} \ex{\|\X\cind{\vvv}\tip{} - \alt{\X}\cind{\vvv}\tip{}\|_\tt}\,d\tt\\
&= \const{1}\int_0^\t \sup_{\v \in V}(\degr+1)\sup_{\vvv\in V} \ex{\|\X\cind{\vvv}\tip{} - \alt{\X}\cind{\vvv}\tip{}\|_\tt}\,d\tt\\
&= (\degr+1)\const{1}\int_0^\t \sup_{\v\in V} \ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\tt}\,d\tt\\
\end{align*}
\end{proof}
\lin
...
\begin{align*}
\sup_{\v\in V}\ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\t} \leq (\degr+1)\const{1}\int_0^\t\sup_{\v\in V} \ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\tt}\,d\tt.
\end{align*}
...
\begin{align*}
\sup_{\v \in V} \ex{\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}{n+1}\|_\T} &\leq (\degr+1)\const{1}\int_0^\T \sup_{\v \in V}\ex{\|\X\cind{\v}\tip{} - \X\cind{\v}\tip{}{n}\|_\tt}\,d\tt \\
...
\[\ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\t} \leq |\sv\cind{\v}\vsi{V} - \alt{\sv}{\v}{V}| + \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\]
...
&\leq \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \sum_{\v \in V}\const{1}\int_0^\t \sum_{\vv \in \cl{\v}} \ex{\b\cind{\v}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&\os{\b\cind{\v}\leq \b(\b\cind{\vv})}{\leq} \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \sum_{\v \in V}\const{1}\int_0^\t \sum_{\vv \in \cl{\v}} \b\ex{\b\cind{\vv}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&= \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \b\const{1}\int_0^\t \sum_{\v \in V}\sum_{\vv \in \cl{\v}} \ex{\b\cind{\vv}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&=\|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \b(\degr+1)\const{1}\int_0^\t \ex{\|\X\cind{}\tip{} - \alt{\X}\cind{}\tip{}\|_{\b,\tt}}\,d\tt\\
\end{align*}

Applying Gronwall's inequality:

\[\ex{\|\X\cind{}\tip{} - \alt{\X}\cind{}\tip{}\|_{\b,\T}} \leq \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b}e^{\T\b(\degr+1)\const{1}}\]
...
&\ind \leq \const{}\int_\S|\s|\int_0^\t\ex{\mb{I}_{\evnt{\t}}\sum_{\vv\in \cl{\v}}\left|\alt{\X}\cind{\vv}\tp{\tt-} - \X\cind{\vv}\tp{\tt-}\right|}\,d\tt\\
&\ind \os{\mb{I}_{\evnt{\t}}\|\alt{\X}\cind{\neigh\vind{\U}}\tip{} - \X\cind{\neigh\vind{\U}}\tip{}\|_\t = 0}{\leq} \left(\int_\S|\s|\,\Sm(d\s)\right)\const{} \int_0^\t \sum_{\vv\in \cl{\v}\cap\U}\ex{\mb{I}_{\evnt{\t}}\|\alt{\X}\cind{\vv}\tip{} - \X\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&\ind \os{\evnt{\tt} \supseteq \evnt{\t}}{\leq} |\S|\const{}(\degr+1) \int_0^\t \sup_{\v\in \U}\ex{\mb{I}_{\evnt{\tt}}\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}\|_\tt}\,d\tt.
\end{align*}

We can conclude that

\[\sup_{\v\in \U}\ex{\left\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}\right\|_{\t}\mb{I}_{\t <\rt{i+1}}} \leq |\S|\const{}(\degr+1)\int_0^\t\sup_{v\in\U}\ex{\left\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}\right\|_\tt\mb{I}_{\tt < \rt{i+1}}}\,d\tt.\]
