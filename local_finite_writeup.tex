...
where \(\{\IGr{\v}:\v\in V\}\) are chosen such that \(\X\cind{}\tip{}\) is well-defined (See Appendix \ref{sec::Infin} for more information. See sections \ref{subsec::Assum:sec::Preli} and \ref{subsec::Well-:sec::Preli} for examples of such \(\IGr{\v}\) relevant to this paper). 
...
\begin{equation}
\X\cind{\v}\tp{\t} = \X\cind{\v}\tp{0} + \int_\S\int_0^\t \s\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)\right],d\s\right) \te{ for all }\v \in V
...
\[\ex{\|\Fpo(\X\cind{}\tip{}){\v} - \Fpo(\alt{\X}{}{}){\v}\|_\t} \leq \ex{|\X\cind{\v}\tp{0} - \alt{\X}{\v}{0}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}{\vv}{}\|_\tt}\,d\tt\]
...
\begin{align*}
\mb{E}\bigg[&\|\Fpo(\X\cind{}\tip{}){\v} - \Fpo(\alt{\X}){\v}\|_\t\bigg] - \ex{|\X\cind{\v}\tp{0} - \alt{\X}{\v}{0}|}\\
&= \ex{\sup_{\ttt \leq \t}\left|\int_\S\int_0^{\ttt} \s\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)\right],d\s\right) -  \int_\S\int_0^{\ttt} \s\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\alt{\X}{}{\tt-},\s)\right],d\s\right)\right|}\\
&=\ex{\sup_{\ttt\leq \t} \left|\int_\S \int_0^{\const{}} \int_0^{\ttt} \s\left(\mb{I}_{\r < \IGr{\v}(\X\cind{}\tp{\tt-},\s)} - \mb{I}_{\r < \IGr{\v}(\alt{\X}{}{\tt-},\s)}\right)\,\poiss{\v}(d\tt\,d\r\,d\s)\right|}\\
&\leq \ex{\int_\S\int_0^{\const{}}\int_0^\t \left|\s\left(\mb{I}_{\r < \IGr{\v}(\X\cind{}\tp{\tt-},\s)} - \mb{I}_{\r < \IGr{\v}(\alt{\X}{}{\tt-},\s)}\right)\right|\,\poiss{\v}(d\tt\,d\r\,d\s)}\\
&=\int_\S\int_0^\t\int_0^{\const{}} |\s|\ex{\mb{I}_{\IGr{\v}(\X\cind{}\tp{\tt-},\s) \leq \r < \IGr{\v}(\alt{\X}{}{\tt-},\s)} + \mb{I}_{\IGr{\v}(\alt{\X}{}{\tt-},\s) \leq \r < \IGr{\v}(\X\cind{}\tp{\tt-},\s)}}\,d\r\,d\tt\,\Sm(d\s)\\
&= \int_\S\int_0^\t|\s|\ex{\left|\IGr{\v}(\X\cind{}\tp{\tt-},\s) - \IGr{\v}(\alt{\X}{}{\tt-},\s)\right|}\,d\tt\,\Sm(d\s)\\
...
\[\sup_{\v \in V} \ex{\left\|\Fpo(\X\cind{}\tip{}){\v} - \Fpo(\alt{\X}{}{}){\v}\right\|_\t} \leq (\degr+1)\const{1}\int_0^\t \sup_{\v \in V} \ex{\|\X\cind{\v}\tip{} - \alt{\X}{\v}{}\|_\tt}\,d\tt.\]
...
\begin{align*}
\sup_{\v \in V} \ex{\left\|\Fpo(\X\cind{}\tip{}){\v} - \Fpo(\alt{\X}{}{}){\v}\right\|_\t} &\leq \sup_{\v\in V}\left(\ex{|\X\cind{\v}\tp{0} - \alt{\X}{\v}{0}|} +  \const{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}{\vv}{}\|_\tt}\,d\tt\right)\\
...
\begin{align*}
\ex{\|\X\cind{\v}\tip{}{1} - \X\cind{\v}\tip{}{0}\|_\t} &\leq \ex{\int_\S\int_0^\t |\s|\,\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\X\cind{}\tp{0},\s)\right],d\s\right)}\\
...
\begin{equation}
\X\cind{\v}\tp{\t} = \sv\cind{\v}\vsi{V} + \int_\S\int_0^\t \s\,\poiss{\v}(d\tt,(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)],d\s).
...
\[\X\cind{\v}\tp{\t+\cdot}{\sv\cind{}\vsi{V}} = \X\cind{\v}\tp{\t}{\sv\cind{}\vsi{V}} + \int_\S\int_\t{\t+\cdot} \s\,\poiss{\v}(d\tt,(0,\IGr{\v}(\X\cind{}\tp{\tt-}{\sv\cind{}\vsi{V}},\s)],d\s).\]

Let \(\alt{\X}{}{}\) be given by the unique solution to the following equation (Proposition \ref{subsec::Well-:Prop::SDEWP}),

\[\alt{\X}{\v}{\cdot} = \X\cind{\v}\tp{\t}{\sv\cind{}\vsi{V}} + \int_\S \int_0^\cdot \s\,\poiss{\v}(d\tt,(0,\IGr{\v}(\alt{\X}{}{\tt-},\s)],d\s).\]
...
\begin{align*}
\X\cind{\v}\tp{\t} &= \sv\cind{\v}\vsi{V} + \int_\S\int_0^\t \s\,\poiss{\v}(d\tt\,(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)],d\s)&\te{ for all } \v \in V\\
...
\[f(\X\cind{}\tp{\t}) = f(\X\cind{}\tp{0}) + \sum_{\v \in V} \int_\S\int_0^\t [f(\X\cind{}\tp{\tt-} + \s\ev\vind{\v}) - f(\X\cind{}\tp{\tt-})]\,\poiss{\v}\left(d\tt,(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)],d\s\right)\]
...
&= \lim_{\t \searrow 0} \ex{\sum_{\v \in V} \frac{1}{\t}\int_\S\int_0^\t \left[f(\X\cind{}\tp{\tt-} + \s\ev\vind{\v}) - f(\X\cind{}\tp{\tt-})\right]\,\poiss{\v}\left(d\tt,(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)],d\s\right)\middle|\X\cind{}\tp{0} = \sv\cind{}\vsi{V}}\\
&\os{f \in \mc{D}}{=} \lim_{\t \searrow 0}\sum_{\v \in V} \frac{1}{\t}\int_\S\int_0^\t \ex{\left[f(\X\cind{}\tp{\tt-} + \s\ev\vind{\v}) - f(\X\cind{}\tp{\tt-})\right]\IGr{\v}(\X\cind{}\tp{\tt-},\s)\middle|\X\cind{}\tp{0} = \sv\cind{}\vsi{V}}d\tt\,\Sm(d\s)\\
...
Here \(\delt\deltf{\f}{\v} = \sup\left\{|f(\sv\cind{}\vsi{V}) - f(\sv\cind{}\vsi{V} + \s\ev\vind{\v}): \sv\cind{}\vsi{V}\in \S\carp{V},\s \in \S\right\}\) as defined in equation \eqref{subsec::Well-:Eqn::fvar}. By Corollary \ref{sec::Infin:Cor::Convergence}, \(\X\cind{}\tip{}{n}\Rightarrow \X\cind{}\tip{}\).
\end{proof}
...
\begin{align}
\X\cind{\v}\tp{\t} &= \X\cind{\v}\tp{0} + \int_0^\t\int_\S \s\,\poiss{\v}(d\s,(0,\crate{}(\X\cind{\{\v,\root\}}\tip{[0,\tt)},\s)],d\tt) \te{ for } \v \in \{1,\dots,\degr\}\nonumber\\
\X\cind{\root}\tp{\t} &= \X\cind{\root}\tp{0} + \int_0^\t\int_\S \s\,\poiss{\root}(d\s,(0,\IGr{\root}(\X\cind{}\tp{\tt-},\s)],d\tt)\nonumber\\
\alt{\m{}{}{}} &= \law(\X\cind{}\tip{})
...
\[\crate{}(\X\cind{\{\v,\root\}}\tip{[0,\t)},\s) = \exmu{\cm{\t}(\X\cind{\{\v,\root\}}\tip{[0,\t)},\alt{\m{}{}{}})}{\IGr{\root}(\X\cind{}\tp{\t-},\s)}\]
...
\begin{equation}
\X\cind{\U}\tip{[0,\rt{i+1})} = \phi\left(\X\cind{\cl{\U}}\tip{[0,\rt{i}]}, \poiss{\U}\left((\rt{i}, \rt{i+1}]\times \mb{R}\times \S\right)\right)
...
\begin{equation}
\X\cind{\UU}\tip{[0,\rtt{j+1})} = \phi\left(\X\cind{\cl{\UU}}\tip{[0,\rtt{j}]}, \poiss{\UU}\left((\rtt{j}, \rtt{j+1}]\times \mb{R}\times \S\right)\right)
...
\begin{align}
\XX{\vv}{\t} &= \X\cind{\vv}\tp{\rt{i}} + \int_\S\int_0^\t \s\alt{\poiss}{\vv}\left(d\tt,\left(0,\IGr{\vv}(\XX{}{\tt-},\s)\right],d\s\right)\te{ for } \vv \in \cl{\U} \te{ on } \rt{i} < \infty \label{sec::Proof1:eqn::alternate SDE}\\
...
&\ind\leq \ex{\mb{I}_{\alt{\evnt}{	}}\left|\int_\S\int{(\rt{i},\t]} \s\,\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)\right]\,d\s\right) - \int_\S\int{(\rt{i},\t]} \s\,\alt{\poiss}{\v}\left(d\tt,\left(0,\IGr{\v}(\alt{\X}{}{\tt-},\s)\right]\,d\s\right)\right|}\\
&\ind \leq \ex{\int_\S\int{\mb{R}}\int{(\rt{i},\t]}|\s|\mb{I}_{\alt{\evnt}{	}}\mb{I}_{\r\in \left(\IGr{\v}(\X\cind{}\tp{\tt-},\s), \IGr{\v}(\alt{\X}{}{\tt-},\s)\right]}\,\poiss{\v}\left(d\tt,d\r,d\s\right)}\\
&\ind \os{\alt{\X}{\cl{\U}}{}[0,\rt{i}] = \X\cind{\cl{\U}}\tip{[0,\rt{i}]}}{=} \ex{\int_\S\int{\mb{R}}\int{(0,\t]}|\s|\mb{I}_{\evnt{\t}}\mb{I}_{\r\in \left(\IGr{\v}(\X\cind{}\tp{\tt-},\s), \IGr{\v}(\alt{\X}{}{\tt-},\s)\right]}\,\poiss{\v}\left(d\tt,d\r,d\s\right)}\\
&\ind = \int_\S\int_\mb{R}\int_0^\t |\s|\ex{\mb{I}_{\evnt{\t}}\mb{I}_{\r\in\left(\IGr{\v}(\X\cind{}\tp{\tt-},\s),\IGr{\v}(\alt{\X}{}{\tt-},\s)\right]}}\,d\tt\,d\r\,\Sm(d\s)\\
&\ind = \int_\S |\s|\int_0^\t \ex{\mb{I}_{\evnt{\t}}\left|\IGr{\v}(\alt{\X}{}{\tt-},\s) - \IGr{\v}(\X\cind{}\tp{\tt-},\s)\right|}\,d\tt\,\Sm(d\s)\\
...
so \(\alt{\X}{\U}{}[0,\rt{i+1}) = \X\cind{\U}\tip{[0,\rt{i+1})}\) almost surely. However, \(\alt{\X}{\U}{}[0,\rt{i+1})\) is uniquely defined by \(\X\cind{\cl{\U}}\tip{[0,\rt{i}]}\) and \(\poiss{\cl{\U}}\left((\rt{i},\rt{i+1}]\times \mb{R}\times \S\right)\).
\end{proof}
...
By Lemma \ref{sec::Proof1:Lem::Decomposition}, we can express \(\X\cind{\U}\tip{[0,\rt{1})}\) in terms of \(\X\cind{\cl{\U}}\tp{0}\) and \(\poiss{\U}([0,\rt{1})\times\mb{R}\times\S)\). We can equivalently describe \(\X\cind{\U}\tip{[0,\rt{1})}\) in terms of \(\X\cind{\U}\tp{0}\), \(\X\cind{\neigh\vind{\U}}\tip{[0,\rt{1})}\) and \(\poiss{\U}([0,\rt{1})\times \mb{R}\times \S)\) which is independent of all terms in expression \eqref{sec::Proof1:eqn::induction_intermediate_1}. Applying a similar argument for \(\X\cind{\UU}\tip{}\), we can apply Lemmas \ref{sec::TL:Lem::Props}(a) and (e) to get
...
Now, \(\X\cind{\U}\tp{\rt{i}}\) is \(\left(\X\cind{\U}\tip{[0,\rt{i})}, \X\cind{\neigh\vind{\U}}\tip{[0,\rt{i})}, \poiss{\U}(\{\rt{i}\}\times [0,\const{}]\times\S)\right)\) measurable. Applying lemmas \ref{sec::TL:Lem::Props} (a) and (e), we get
...
\pfsum \(\proj\vsi{\tree{1}}\ts{\T}(\X\cind{}\tip{})\) can be expressed as a sum of a random variable and a non-explosive marked point process. The trick is to find a \(\proj\vsi{\tree{1}}\ts{\T}(\X\cind{}\tip{})\)-measurable intensity for that point process. However, theorems that allow us to do that require that the candidate intensity have certain measurability properties, so we begin by proving those.
...
\[\X\cind{\v}\tp{\t} = \X\cind{\v}\tp{0} + \int_\S\int_0^\t \s\,\poiss{\v}\left(d\tt,\left(0,c{\v}(\X\cind{}\tp{\tt-},\s)\right],d\s\right) \te{ for all } \v \in V.\]

Then,

\[\left(\proj\vsi{\tree\sln{1}}\ts{\T}(\X\cind{}\tip{})\right)^\t = \left(\proj\vsi{\tree\sln{1}}\ts{\T}(\X\cind{}\tip{})\right)^0 + \sum_{\v \in \tree\sln{1}}\ev\vind{\v}\int_\S\int_0^\t \s\,\poiss{\v}\left(d\tt,\left(0,\IGr{\v}(\X\cind{}\tp{\tt-},\s)\right],d\s\right).\]
...
Therefore it suffices to derive the \(\F\vsi{\tree\sln{1}}\ts{\t}\)-intensity of \(\rp{}\). \(\rp{}\) has \(\F\vsi{V}\ts{\t}\)-intensity \(\rate{\rp{}}(\X\cind{}\tp{\t-},\mark{}) := \sum_{\v \in \tree{1}}\sum_{\s\in \S}\mb{I}_{\mark{} = \s\ev\vind{\v}}\IGr{\v}(\X\cind{}\tp{\t-},\s)\). Each \(\IGr{\v}\) is clearly left-continuous in time, so \(\rate{\rp{}}\) is also left-continuous in time. Recall that \(\m{}{}{}\) is the law of \(\X\cind{}\tip{}\). Then,

\[\exmu{\m{}{}{}}{\rate{\rp{}}(\X\cind{}\tp{\t-},\mark{})|\F\vsi{\tree\sln{1}}\ts{\t-}} = \sum_{\v \in \tree\sln{1}}\sum_{\s\in \S} \mb{I}_{\mark{} = \s\ev\vind{\v}}\exmu{\m{}{}{}}{\IGr{\v}(\X\cind{}\tp{\t-},\s)|\F\vsi{\tree\sln{1}}\ts{\t-}}.\]

Applying Theorem \ref{sec::Main:Thm::CI} and symmetry of \(\m{}{}{}\) yields,

\begin{align*}
\exmu{\m{}{}{}}{\IGr{\v}(\X\cind{}\tp{\t-},\s)|\F\vsi{\tree\sln{1}}\ts{\t-}} &= \exmu{\m{}{}{}}{\IGr{\v}(\X\cind{}\tp{\t-},\s)|\X\cind{\tree\sln{1}}\tip{[0,\t)}}\\
...
\begin{align*}
\X\cind{\v}\tp{\t} &= \X\cind{\v}\tp{0} + \int_0^\t\int_\S \s\,\poiss{\v}(d\s,(0,\crate{\v}{\tt}(\s)],d\tt) \te{ for } \v \in \{1,\dots,\degr\},\\
\X\cind{\root}\tp{\t} &= \X\cind{\root}\tp{0} + \int_0^\t\int_\S \s\,\poiss{\root}(d\s,(0,\IGr{\root}(\X\cind{}\tp{\tt-},\s)],d\tt).
\end{align*}
\end{proof}
...
\begin{align}
\X\cind{\{\v,\root\}}\tp{\t}{1} = \X\cind{\{\v,\root\}}\tp{0}{1} &+ \ev\vind{\v}\int_\S\int{(0,\crate{}{}(\X\cind{\{\v,\root\}}\tip{[0,\tt)}{1},\s)]}\int{(0,\t]}\s\,\poiss{\v}(d\tt,d\r,d\s)\nonumber\\
&+ \ev\vind{\root}\int_\S\int{(0,\crate{}{}(\X\cind{\{\root,\v\}}\tip{[0,\tt)}{1},\s)]}\int{(0,\t]}\s\,\poiss{\root}(d\tt,d\r,d\s)
...
\[\crate{\rp{}}{\t}(\X\cind{}\tip{[0,\t)}{1},\mark{}) = \sum_{\v\in \tree\sln{1}\setminus\{\root\}}\sum_{\s\in \S} \mb{I}_{\mark{} = \s\ev\vind{\v}}\crate{}{}(\X\cind{\{\v,\root\}}\tip{[0,\t)}{1},\s) + \sum_{\s\in\S}\mb{I}_{\mark{} = \s\ev\vind{\root}}\IGr{\root}(\X\cind{}\tp{\t-}{1},\s).\]

Then \(\proj\vsi{\{\v,\root\}}\ts{\T}(\rp{})\defeq \pmap{\{\v,\root\}}(\X\cind{}\tip{}{1})\) \tr{notation?} has \(\F\vsi{\tree\sln{1}}\ts{}\)-predictable intensity,

\[\rate{\proj\vsi{\{\v,\root\}}\ts{\T}(\rp{})}{\t}(\X\cind{}\tip{[0,\t)}{1},\mark{}) \defeq \sum_{\s\in \S} \left(\mb{I}_{\mark{} = \s\ev\vind{\v}}\crate{}{}(\X\cind{\{\v,\root\}}\tip{[0,\t)}{1},\s) + \mb{I}_{\mark{} = \s\ev\vind{\root}}\IGr{\root}(\X\cind{}\tp{\t-}{1},\s)\right)\te{ for } \kappa \in \S\carp{\{\v,\root\}}.\]
...
&=\exmu{\m{}{}{1}}{\sum_{\s\in \S}\left(\mb{I}_{\kappa = \s\ev\vind{\v}} \crate{}{}(\X\cind{\{\v,\root\}}\tip{[0,\t)}{1},\s) + \mb{I}_{\kappa = \s\ev\vind{\root}}\IGr{\root}(\X\cind{}\tp{\t-}{1},\s)\right)\middle|\F\vsi{\{\v,\root\}}\ts{\t-}}\\
&=\sum_{\s\in \S}\mb{I}_{\kappa = \s\ev\vind{\v}} \crate{}{}(\X\cind{\{\v,\root\}}\tip{[0,\t)}{1},\s) + \sum_{\s\in \S}\mb{I}_{\kappa = \s\ev\vind{\root}}\exmu{\m{}{}{1}}{\IGr{\root}(\X\cind{}\tp{\t-}{1},\s)\middle|\F\vsi{\{\v,\root\}}\ts{\t-}}\\
...
\X\cind{\v}\tp{0} + \int{(0,\t]}\int_\S\int{\left(0,\crate{}{}(\X\cind{\{\v,\p{\v}\}}\tip{[0,\tt)},\s)\right]}\s\,\poiss{\v}(d\r,d\s,d\tt)\te{ if } \v \in \tree\sln{n}\setminus \tree\sln{n-1}\\
\X\cind{\v}\tp{0} + \int{(0,\t]}\int_\S\int{\left(0,\rate{}(\X\cind{\cl{\v}}\tp{\tt-},\s\right)}\s\,\poiss{\v}(d\r,d\s,d\tt)\te{ if } \v \in \tree\sln{n-1}
\end{cases}
...
Let \(\{\poiss{\v},\alt{\poiss}{\v}:\v\in V\}\) be constructed as above. Then \(\X\cind{}\tip{}{n}\) almost surely solves the equation,
...
\X\cind{\v}\tp{0}{} + \int{(0,\t]}\int_\S\int{(0,\rate{}(\X\cind{\cl{\v}}\tp{\tt-}{n},\s)]}\s\,\poiss{\v}(d\tt,d\s,d\r)\te{ for } \v \in \tree\sln{n-1}.
\end{cases}
...
\begin{align*}
\X\cind{\v}\tp{0} + \int{(0,\t]}\int_\S\int{(0,\rate{}(\X\cind{\cl{\v}}\tp{\tt-},\s)]}\s\poiss{\v}(d\r,d\s,d\tt) &= \X\cind{\v}\tp{0} + \int{(0,\t]}\int_\S\int{(0,\rate{}(\X\cind{\cl{\v}}\tp{\tt-}{n},\s)]}\s\poiss{\v}(d\r,d\s,d\tt)\\
...
\begin{equation}
\X\cind{\v}\tp{\t} = \X\cind{\v}\tp{0} + \int{(0,\t]}\int_\S \int{(0,\rate{}(\X\cind{}\tp{\tt-},\s)]}\s\,\poiss{\v}(d\r,d\s,d\tt).
...
\begin{equation}
\X\cind{\v}\tp{\t} = \X\cind{0}\tp{\v} + \int{(0,\t]}\int_\S\int{(0,\ratee{\tt,\v}(\X\cind{}\tip{[0,\tt)},\s)]}  \s\,\poiss{\v}(d\r,d\s,d\tt)
...
\[\X\cind{\v}\tp{\t} = \X\cind{\v}\tp{0} +\int{(0,\t]}\int_\S\int{(0,1]} \s \poiss{\v}(d\r,d\s,d\tt),\]
...
\[\X\cind{\v}\tp{\t}{n} = \X\cind{\v}\tp{0}{n} + \int_\S\int_0^\t \s\,\poiss{\v}(d\tt,(0,\rate{\v}(\X\cind{}\tp{\tt-}{n})],d\s)\]

for all \(\v\in V\). Let

\[\alt{\poiss}{\v}(\cdot) = \int_\S\,\poiss{\v}(\cdot\times \S).\]

Since \(\Sm\) is a probability measure on \(\S\) and \(\{\poiss{\v}:\v\in V\}\) are i.i.d., it follows that \(\{\alt{\poiss}{\v}:\v\in \U\}\) is a sequence of unit rate i.i.d. Poisson processes as desired. Under this coupling, every time \(\proj\vsi{\U}\ts{\T}(\X\cind{}\tip{}{n})\) experiences a jump, \(\XX{}{}\) also experiences a jump. Furthermore, the magnitude of the jump of \(\XX{}{}\) is larger. For any \(\tt<\ttt\in [0,\T]\) and \(\v\in \U\),
...
\begin{align*}
\pr\left(\poiss{n}\left(\inte\times R\times \alt{\inte}\right) = 0\right) &= \sum_{\sv\cind{}\vsi{\tree\sln{n}} \in \alt{\S}^n} \pr\left(\poiss{n}^{\sv\cind{}\vsi{\tree\sln{n}}}\left(\inte\times R\times \alt{\inte}\right) = 0\right)\pr(\X\cind{}\tp{0}{n} = \sv\cind{}\vsi{\tree\sln{n}})\\
