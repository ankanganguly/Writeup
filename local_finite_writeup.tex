...
\begin{equation}
\cm\ts{\t}(\x\cind{\{\root,1\}}\tip{[0,\t)},\mmm\sln{}\ts{})(\cdot) = \pr\me{\mmm\sln{}\ts{}}\left(\X\cind{\{2,\dots,\degr\}}\tip{[0,\t)} \in \cdot|\X\cind{\{\root,1\}}\tip{[0,\t)} = \x\cind{\{\root,1\}}\tip{[0,\t)}\right).
...
&\ind\leq \ex{\mb{I}_{\alt{\evnt}\ts{\t}}\left|\int_\S\int_{(\rt\indx{i},\t]} \s\,\poiss\vind{\v}\left(d\tt,\left(0,\IGr\vind{\v}(\X\cind{}\tp{\tt-},\s)\right]\,d\s\right) - \int_\S\int_{(\rt\indx{i},\t]} \s\,\alt{\poiss}\vind{\v}\left(d\tt,\left(0,\IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)\right]\,d\s\right)\right|}\\
&\ind \leq \ex{\int_\S\int_{\mb{R}}\int_{(\rt\indx{i},\t]}|\s|\mb{I}_{\alt{\evnt}\ts{\t}}\mb{I}_{\r\in \left(\IGr\vind{\v}(\X\cind{}\tp{\tt-},\s), \IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)\right]}\,\poiss\vind{\v}\left(d\tt,d\r,d\s\right)}\\
&\ind \os{\alt{\X}\cind{\cl{\U}}\tip{[0,\rt\indx{i}]} = \X\cind{\cl{\U}}\tip{[0,\rt\indx{i}]}}{=} \ex{\int_\S\int_{\mb{R}}\int_{(0,\t]}|\s|\mb{I}_{\evnt\ts{\t}}\mb{I}_{\r\in \left(\IGr\vind{\v}(\X\cind{}\tp{\tt-},\s), \IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)\right]}\,\poiss\vind{\v}\left(d\tt,d\r,d\s\right)}\\
...
However, this is not \(\F\vsi{\tree\sln{1}}\ts{\t}\)-adapted. Let \(\rp = \{(\rt\indx{\it},\mark\indx{\it})\} = \pmap\vind{\tree\sln{1}}(\X\cind{}\tip{}).\) Then,

\[\left(\proj\vsi{\tree\sln{1}}\ts{\T}(\X\cind{}\tip{})\right)^\t = \left(\proj\vsi{\tree\sln{1}}\ts{\T}(\X\cind{}\tip{})\right)^0 + \sum_{0 < \rt\indx{\it} \leq \t}\mark\indx{\it}\]
...
\begin{align}
\X\sln{1}\cind{\{\v,\root\}}\tp{\t} = \X\sln{1}\cind{\{\v,\root\}}\tp{0} &+ \ev\vind{\v}\int_\S\int_{(0,\crate\vind{}\ts{}(\X\sln{1}\cind{\{\v,\root\}}\tip{[0,\tt)},\s)]}\int_{(0,\t]}\s\,\poiss\vind{\v}(d\tt,d\r,d\s)\nonumber\\
&+ \ev\vind{\root}\int_\S\int_{(0,\crate\vind{}\ts{}(\X\sln{1}\cind{\{\root,\v\}}\tip{[0,\tt)},\s)]}\int_{(0,\t]}\s\,\poiss\vind{\root}(d\tt,d\r,d\s)
...
\[\XX\cind{}\tp{\t} = \XX\cind{}\tp{0} + \int_{(0,\t]}\int_{(0,1]}\int_\S\s\,\poiss\vind{}(d\s,d\r,d\tt).\]
...
\begin{lem}
For any \(\v \in V\setminus\{\root\}\), let \(\p\vind{\v}\) be the parent node of \(\v\) (neighboring node of \(\v\) closest to \(\root\)). Fix \(n \in \mb{N}\). Let \(\mmm\sln{n}\ts{} \in \pmsr\left(\sset\vsi{\tree\sln{n}}\ts{\T-}\right)\) be the law of the unique solution to the following SDE:

\begin{equation}
\X\cind{\v}\tp{\t} = \begin{cases}
\X\cind{\v}\tp{0} + \int_{(0,\t]}\int_\S\int_{\left(0,\crate\vind{}\ts{}(\X\cind{\{\v,\p\vind{\v}\}}\tip{[0,\tt)},\s)\right]}\s\,\poiss\vind{\v}(d\r,d\s,d\tt)\te{ if } \v \in \tree\sln{n}\setminus \tree\sln{n-1}\\
...
\begin{align}
\ds\vind{\v}\ts{\t}(\XXX\cind{}\tip{},f) &\defeq \sum_{0 < \rt\indx{\it}\vind{\v} \leq \t} \log{f(\XXX\cind{}\tip{[0,\rt\indx{\it}\vind{\v})},\mark\indx{\it}\vind{\v})} - \int_{(0,\t]\times \S} [f(\XXX\cind{}\tip{[0,\tt)},\s) - 1]\,ds\,\Sm(d\s) \label{subsec::ProofU:eqn::shortdense}\\
f&\te{ is a }[0,\const\indx{}] \te{-valued mapping and } \pmap\vind{\v}(\XXX\cind{}\tip{}) = \{(\rt\indx{\it}\vind{\v},\mark\indx{\it}\vind{\v}):\it\in\mb{N}\}.\nonumber
\end{align} 
\labe{subsec::ProofU:eqn::shortdense}

Let \(\{(\rt\indx{\it},\mark\indx{\it})\}\) be the events of \(\pmap\vind{}(\X\sln{1}\cind{}\tip{})\). By Lemma \ref{subsec::Some:Lem::radnik},

\begin{align}
\dense\sln{1}\ts{\t}(\X\cind{\tree\sln{1}}\tip{})&\defeq \frac{d\mmm\sln{1}\ts{\t}}{d\mm\sln{1}\ts{\t}}\nonumber\\
&= \frac{d\mmm\sln{1}\ts{0}}{d\mm\sln{1}\ts{0}}\exp\left(\sum_{0< \rt\indx{\it} \leq \t} \log{\ratee(\X\sln{1}\cind{}\tip{[0,\rt\indx{\it})},\mark\indx{\it})} - \int_{(0,\t]\times\S} [\ratee(\X\sln{1}\cind{}\tip{[0,\tt)},\s) - 1]\,d\tt\,\Sm(d\s)\right)\nonumber\\
&= \frac{d\mmm\sln{1}\ts{0}}{d\mm\sln{1}\ts{0}}\exp\Bigg(\sum_{\v\in\tree\sln{1}\setminus\{\root\}}\left(\sum_{0< \rt\indx{\it}\vind{\v} \leq \t} \log{\crate\vind{}\ts{}(\X\sln{1}\cind{\{\v,\root\}}\tip{[0,\rt\indx{\it}\vind{\v})},\mark\indx{\it})} - \int_{(0,\t]\times\S} [\crate\vind{}\ts{}(\X\sln{1}\cind{\{\v,\root\}}\tip{[0,\tt)},\s) - 1]\,d\tt\,\Sm(d\s)\right)\nonumber\\
...
&= \frac{d\mmm\sln{1}\ts{0}}{d\mm\sln{1}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{1}\setminus\{\root\}} \ds\vind{\v}\ts{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{},\crate\vind{}\ts{}) + \ds\vind{\root}\ts{\t}(\X\sln{1}\cind{\tree\sln{1}}\tip{},\IGrg{})\right).
...
\begin{equation}
\alt{\dense}\ts{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{}) \defeq \frac{d\mmm\vind{\{\v,\root\}}\ts{\t}}{d\mm\vind{\{\v,\root\}}\ts{\t}} = \frac{d\mmm\vind{\{\v,\root\}}\ts{0}}{d\mm\vind{\{\v,\root\}}\ts{0}}\exp\left(\ds\vind{\v}\ts{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{},\crate\vind{}\ts{}) + \ds\vind{\root}\ts{\t}(\X\sln{1}\cind{\{\root,\v\}}\tip{},\crate\vind{}\ts{})\right).
...
\begin{align*}
\cdense\ts{\t}&(\X\sln{1}\cind{\{\tree\sln{1}\setminus\{\root\}\}}\tip{};\x\cind{\{\root,1\}}\tip{})\\
& \defeq\frac{d\mmm\vind{\tree\sln{1}\setminus\{\root,1\}}\ts{0}}{d\mm\vind{\tree\sln{1}\setminus\{\root,1\}}\ts{0}}\exp\left(\sum_{\v\in \tree\sln{1}\setminus\{\root,1\}} \ds\vind{\v}\ts{\t}([\X\sln{1}\cind{\{v\}}\tip{},\x\cind{\root}\tip{}],\crate\vind{}\ts{}) + \ds\vind{\root}\ts{\t}([\X\sln{1}\cind{\tree\sln{1}\setminus\{\root,1\}}\tip{},\x\cind{\{\root,1\}}\tip{}],\IGrg{}) - \ds\vind{\root}\ts{\t}(\x\cind{\{\root,1\}}\tip{},\crate\vind{}\ts{})\right)\\
\end{align*}

Then,

\[\cdense\ts{\t}(\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}}\tip{};\x\cind{\{\root,1\}}\tip{}) = \frac{\dense\sln{1}\ts{\t}([\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}}\tip{},\x\cind{\{\root,1\}}\tip{}])}{\alt{\dense\sln{}\ts{\t}}(\x\cind{\{1,\root\}}\tip{})}.\]

By definition,

\[\alt{\dense}\ts{\t}(\X\sln{1}\cind{\{1,\root\}}\tip{}) = \exmu{\mm\vind{\tree\sln{1}\setminus\{1,\root\}}\ts{\t}(\X\sln{1}\cind{\tree\sln{1}\setminus\{1,\root\}}\tip{})}{\dense\sln{1}\ts{\t}([\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}}\tip{},\x\cind{\{\root,1\}}\tip{}])}.\]

So,

\[\exmu{\mm\vind{\tree\sln{1}\setminus\{1,\root\}}\ts{\t}}{\cdense\ts{\t}(\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}}\tip{};\x\cind{\{1,\root\}}\tip{})} = 1.\]
...
\ind \tr{(These are not functions anymore)} Let \(\v \in V\setminus\{\root\}\). Let \(\c\vind{}:V\setminus\{\root\} \ra V^{\degr-1}\) be defined such that \(\c\vind{\v} = (\c\cind{1}\vind{\v},\dots,\c\cind{\degr-1}\vind{\v})\) are the children of \(\v\). Let \(\p\vind{}:V\setminus\{\root\}\ra V\) map nodes to their parent.

\ind We can define the sequence of functions \(\dense\sln{n}\ts{\t}: \sset\vsi{\tree\sln{n}}\ts{\t-} \ra [0,\infty)\) by,

\[\dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{}) := \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{})\prod_{\v\in \tree\sln{n-1}\setminus\tree\sln{n-2}} \cdense\ts{\t}(\X\sln{n}\cind{\c\vind{\v}}\tip{};\X\sln{n}\cind{\{\v,\root\}}\tip{})\]
...
\begin{equation}
\dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{}) = \frac{d\mmm\sln{n}\ts{\t}}{d\mm\sln{n}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{n-1}}\ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n}\setminus\tree\sln{n-1}} \ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\{v,\p\vind{\v}\}}\tip{},\crate\vind{}\ts{})\right).
...
\begin{align*}
\dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{}) &= \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{})\prod_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}} \cdense\ts{\t}(\X\sln{2}\cind{\c\vind{\v}}\tip{};\X\sln{2}\cind{\{\v,\p\vind{\v}\}}\tip{})\\
&=\left(\frac{d\mmm\sln{n-1}\ts{0}}{d\mm\sln{n-1}\ts{0}}\prod_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}}\frac{d\mmm\vind{\c\vind{\v}}\ts{0}}{d\mm\vind{\c\vind{\v}}\ts{0}}\right)\exp\Bigg(\sum_{\v\in\tree\sln{n-2}}\ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}} \ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{},\crate\vind{}\ts{})\\
&\ind + \sum_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}}\left(\sum_{\vv\in \c\vind{\v}} \ds\vind{\vv}\ts{\t}(\X\sln{n}\cind{\{\vv,\v\}}\tip{},\crate\vind{}\ts{}) + \ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) - \ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{},\crate\vind{}\ts{})\right)\Bigg)\\
&=\frac{d\mmm\sln{n}\ts{0}}{d\mm\sln{n}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{n-1}}\ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n}\setminus\tree\sln{n-1}} \ds\vind{\v}\ts{\t}(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{},\crate\vind{}\ts{})\right).
\end{align*}
...
\[\frac{d\mmm\sln{n}\ts{\t}}{d\mm\sln{n}\ts{\t}} = \dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{}).\]

Then,

\begin{align*}
\frac{d\proj\vsi{\tree\sln{n-1}}\ts{\t}(\mmm\sln{n}\ts{\t})}{d\mm\sln{n-1}\ts{\t}} &= \int_{\sset\vsi{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t-}} \frac{d\mmm\sln{n}\ts{\t}}{d\mm\sln{n}\ts{\t}}\,\mm\vind{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t}(d\X\sln{n}\cind{\tree\sln{n}\setminus\tree\sln{n-1}}\tip{})\\
&=\int_{\sset\vsi{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t-}} \dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{})\,\mm\vind{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t}(d\X\sln{n}\cind{\tree\sln{n}\setminus\tree\sln{n-1}}\tip{})\\
&= \int_{\sset\vsi{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t-}} \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{})\prod_{\v\in \tree\sln{n-1}\setminus\tree\sln{n-2}} \cdense\ts{\t}(\X\sln{n}\cind{\c\vind{\v}}\tip{};\X\sln{n}\cind{\{v,\p\vind{\v}\}}\tip{})\,\mm\vind{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t}(d\X\sln{n}\cind{\tree\sln{n}\setminus\tree\sln{n-1}}\tip{})\\
&= \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{})\prod_{\v\in \tree\sln{n-1}\setminus\tree\sln{n-2}}\int_{\sset\vsi{\c\vind{\v}}\ts{\t-}} \cdense\ts{\t}(\X\sln{n}\cind{\c\vind{\v}}\tip{};\X\sln{n}\cind{\{v,\p\vind{\v}\}}\tip{})\,\mm\vind{\c\vind{\v}}\ts{\t}(d\X\sln{n}\cind{\c\vind{\v}}\tip{})\\
&= \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{})\prod_{\v\in \tree\sln{n-1}\setminus\tree\sln{n-2}}\exmu{\mm\vind{\c\vind{\v}}\ts{\t}}{\cdense\ts{\t}(\X\sln{n}\cind{\c\vind{\v}}\tip{};\X\sln{n}\cind{\{v,\p\vind{\v}\}}\tip{})}\\
&= \dense\sln{n-1}\ts{\t}(\X\sln{n}\cind{\tree\sln{n-1}}\tip{}).
\end{align*}
...
\item \(\frac{d\mmm\sln{n}\ts{\t}}{d\mm\sln{n}\ts{\t}} = \dense\sln{n}\ts{\t}(\X\sln{n}\cind{}\tip{})\).
\end{enumerate}
...
\crate\vind{}\ts{}(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{[0,\t)},\s) &\te{ if } \sv\cind{}\vsi{\tree\sln{n}} = \s\ev\vind{\v},\s\in\S\setminus\{0\},\v\in\tree\sln{n}\setminus\tree\sln{n-1}\\
0 &\te{ otherwise.}
\end{cases}\]
...
\begin{equation}
\poiss\sln{n}(\evnt) = \alt{\poiss}\sln{n}\left(\evnt\cap\{(\t,\sv\cind{}\vsi{\tree\sln{n}},\r):\r > \rate\sln{n}(\t,\sv\cind{}\vsi{\tree\sln{n}})\}\right) + \#\{(\rt\slnvind{n}{\v}\indx{\it},\mark\indx{\it}\slnvind{n}{\v},\rv\vind{\v}\indx{\it}\rate\sln{n}(\rt\slnvind{n}{\v}\indx{\it},\mark\indx{\it}\slnvind{n}{\v}) \in \evnt: \v\in \tree\sln{n}\}.
...
\X\sln{}\cind{\v}\tp{0} + \int_{(0,\t]}\int_\S\int_{(0,\crate\vind{}\ts{}(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{[0,\tt)},\s)]}\s\alt{\poiss}\vind{\v}(d\tt,d\s,d\r)\te{ for } \v \in \tree\sln{n}\setminus\tree\sln{n-1}\\
\X\sln{}\cind{\v}\tp{0} + \int_{(0,\t]}\int_\S\int_{(0,\rate(\X\sln{n}\cind{\cl{\v}}\tp{\tt-},\s)]}\s\,\poiss\vind{\v}(d\tt,d\s,d\r)\te{ for } \v \in \tree\sln{n-1}.
\end{cases}
...
\begin{equation}
\X\cind{\v}\tp{\t} = \X\cind{\v}\tp{0} + \int_{(0,\t]}\int_\S \int_{(0,\rate(\X\cind{}\tp{\tt-},\s)]}\s\,\poiss\vind{\v}(d\r,d\s,d\tt).
...
\begin{enumerate}[(a)]
\item \(\pr\me{\x}(\X\cind{}\tp{0} = \x) = 1\) for all \(\x \in \spce\).

\item The mapping \(\x \mapsto \pr\me{\x}(\typset)\) is measurable for every \(\sigma(\X)\)-measurable set \(\typset\).

\item \(\pr\me{\x}(\X\cind{}\tp{\tt+\cdot} \in \typset|\X\cind{}\tp{\ttt}:\ttt \in [0,\tt]) = \pr\me{\X\cind{}\tp{\tt}}(\typset)\) for all \(\typset \in \sigma(\X)\) and \(\x \in \spce\).
...
\begin{align*}
\exmu{\m\sln{n}}{f(\XX\cind{}\tp{\t})} & = \exmu{\mm\sln{n}}{\exmu{\m\sln{n}}{f(\XX\cind{}\tp{\t})|\XX\cind{}\tp{0}}} \ra \exmu{\mm\sln{n}}{\exmu{\m}{f(\XX\cind{}\tp{\t})|\XX\cind{}\tp{0}}}\\
& \ra \exmu{\mm}{\exmu{\m}{f(\XX\cind{}\tp{\t})|\XX\cind{}\tp{0}}} = \exmu{\m}{f(\XX\cind{}\tp{\t})}
\end{align*}
...
\begin{align}
\exmu{\m\sln{n}}{f(\XX\cind{}\tp{0},\XX\cind{}\tp{\t\indx{2}},\dots,\XX\cind{}\tp{\t\indx{\it}})|\XX\cind{}\tp{0} = \xxx}&= \exmu{\m\sln{n}}{f\indx{1}(\XX\cind{}\tp{0})f\indx{2}(\XX\cind{}\tp{\t\indx{2}},\dots,\XX\cind{}\tp{\t\indx{k}})|\XX\cind{}\tp{0}=\xxx}\nonumber\\
&= f\indx{1}(\xxx)\exmu{\m\sln{n}}{f\indx{2}(\XX\cind{}\tp{\t\indx{2}},\dots,\XX\cind{}\tp{\t\indx{k}})|\XX\cind{}\tp{0}=\xxx}\nonumber\\
&\os{\te{Induction}}{\ra} f\indx{1}(\xxx)\exmu{\m}{f\indx{2}(\XX\cind{}\tp{\t\indx{2}},\dots,\XX\cind{}\tp{\t\indx{k}})|\XX\cind{}\tp{0}=\xxx}\nonumber\\
...
\begin{align*}
\mb{E}\me{\m\sln{n}}\bigg[f(\XX\cind{}\tp{\t\indx{1}},\dots,\XX\cind{}\tp{\t\indx{\it}})&|\XX\cind{}\tp{0}=\xxx\bigg]\\
&=\exmu{\XX\sim\m\sln{n}}{\exmu{\XX\sim \m\sln{n}}{f(\XX\tp{\t\indx{1}},\dots,\XX\tp{\t\indx{\it}})|\XX\tp{\t\indx{1}}=\XX\cind{}\tp{\t\indx{1}},\XX\tp{0}=\xxx}|\XX\cind{}\tp{0}=\xxx}\\
&=\exmu{\XX\sim\m\sln{n}}{\exmu{\XX\sim \m\sln{n}}{f(\XX\tp{0},\dots,\XX\tp{\t\indx{\it}-\t\indx{1}})|\XX\tp{0}=\XX\cind{}\tp{\t\indx{1}}}|\XX\cind{}\tp{0}=\xxx}\\
&\os{\te{Eq \eqref{sec::Infin:eqn::zerocase}}}{\ra} \exmu{\XX\sim\m\sln{n}}{\exmu{\XX\sim \m}{f(\XX\tp{0},\dots,\XX\tp{\t\indx{\it}-\t\indx{1}})|\XX\tp{0}=\XX\cind{}\tp{\t\indx{1}}}|\XX\cind{}\tp{0}=\xxx}\\
&\os{\te{Thm \ref{sec::Infin:Thm::TrotterKurtz}}}{\ra} \exmu{\XX\sim\m}{\exmu{\XX\sim \m}{f(\XX\tp{0},\dots,\XX\tp{\t\indx{\it}-\t\indx{1}})|\XX\tp{0}=\XX\cind{}\tp{\t\indx{1}}}|\XX\cind{}\tp{0}=\xxx}\\
...
\begin{equation}
\X\cind{\v}\tp{\t} = \X\cind{0}\tp{\v} + \int_{(0,\t]}\int_\S\int_{(0,\ratee\ts{\tt}\vind{\v}(\X\cind{}\tip{[0,\tt)},\s)]}  \s\,\poiss\vind{\v}(d\r,d\s,d\tt)
...
Thus, \(\sum_{\v\in V}\poiss\vind{\v}\) is an almost surely finite measure on \([0,\T)\times \S\times [0,\const\indx{}]\). Let \(\{(\rt\indx{\it},\mark\indx{\it},\r{\it},\v{\it}):\it = 0,\dots,n, n\in \mb{N}\}\) be the set of events of \(\sum_{\v\in V}\) in \([0,\T)\times\S\times [0,\const\indx{})\times V\) ordered so that \(0=\rt\indx{0} < \rt\indx{1} < \cdots < \rt\indx{n} < \T\). In each event, \(\rt\indx{\it} \in [0,\T)\), \(\mark\indx{\it}\in \S\setminus\{0\}, \r{\it} \in [0,\const\indx{}]\) and \(\v{\it} \in V\) denotes which process went off.
...
\X\cind{}\tp{0} + \mark\indx{1}\ev\vind{\v{1}} &\te{ if } \r{1} \leq \ratee\ts{\rt\indx{1}}\vind{\v{1}}(\X\cind{}\tip{[0,\rt\indx{1})},\mark\indx{1})\\
...
\X\cind{}\tp{\rt\indx{\it-1}} + \mark\indx{\it}\ev\vind{\v{\it}} &\te{ if } \r{\it} \leq \ratee\ts{\rt\indx{\it}}\vind{\v{\it}}(\X\cind{}\tip{[0,\rt\indx{\it})},\mark\indx{\it})\\
...
\XX\cind{}\tp{0} + \mark\indx{1}\ev\vind{\v{1}} &\te{ if } \r{1} \leq \ratee\ts{\rt\indx{1}}\vind{\v{1}}(\XX\cind{}\tip{[0,\rt\indx{1})},\mark\indx{1})\\
...
\XX\cind{}\tp{\rt\indx{\it-1}} + \mark\indx{\it}\ev\vind{\v{\it}} &\te{ if } \r{\it} \leq \ratee\ts{\rt\indx{\it}}\vind{\v{\it}}(\XX\cind{}\tip{[0,\rt\indx{\it})},\mark\indx{\it})\\
...
We can assume without loss of generality that \(\ratee\ts{\t}\vind{\v}(\xx\tip{[0,\t)},\s) = 0\) for all \(\xx\tip{[0,\t)}\in \sset\vsi{}\ts{\t-}\) such that \(\pr(\XX\cind{}\tip{[0,\t)} = \xx\tip{[0,\t)}) = 0\). Then since \(\ratee\ts{\t}(\cdot)\) is almost surely left-continuous, we can conclude that \(\ratee\ts{\t}\vind{\v}(\cdot,\s)\) is left-continuous with respect to \(\t\) given c\`adl\`ag inputs for all \(\v\in V,\s\in\S\). By Lemma \ref{subsec::Some:Lem::genfiniteSDE}, the following equation has a unique solution:
...
where \(\{\X\cind{\v}\tp{0}\}_{\v \in V}\) are i.i.d uniformly distributed over \(\S\). Let \(\rp \defeq \pmap\vind{}(\X\cind{}\tip{}) = \{(\rt\indx{\it},\mark\indx{\it}):\it\in \mb{N}\}\).
...
\begin{equation}
\frac{d\m\sln{}\ts{\t}}{d\mm\vind{}\ts{\t}}= \frac{d\m\sln{}\ts{0}}{d\mm\vind{}\ts{0}}\left(\prod_{0< \rt\indx{\it}\leq \t} \ratee\ts{\t}(\mark\indx{\it})\right)\exp\left(-\int_{(0,\t]\times \S} (\ratee\ts{\tt}(\s) - 1)\,d\tt\alt{\Sm}(d\s)\right)
...
\begin{proof}
Notice that \(\X\cind{}\tip{} \mapsto (\X\cind{}\tp{0},\pmap(\X\cind{}\tip{}))\) is a one-to-one mapping from c\`adl\`ag processes on \(\S\carp{V}\) to marked point processes with marks in \(\alt{\S}\). Thus, we can consider \(\rp\) a marked point process with marks in \(\alt{\S}\). In both cases, \(\rp\) is \(\F\vsi{}\ts{\t}\)-adapted, where \(\F\vsi{}\ts{\t} = \F\vsi{}\ts{0}\vee \FH\ts{\t}\) and \(\FH\ts{\t}\) is the natural filtration of \(\rp\).
