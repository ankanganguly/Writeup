...
\begin{equation}
\sup_{\v\in V,\s \in \S,\sv\vsi{V}\in \S\carp{V}} \IGr\vind{\v}(\sv\vsi{V},\s) = \const\indx{}.
...
\[\left|\sum_{\ss \in \SS} (\IGr\vind{\v}(\sv\vsi{V},\ss) - \IGr\vind{\v}(\sv\vsi{V}+\s\ev\vind{\vv},\ss))\right| \leq \const\indx{}|\S| \te{ for all } \sv\vsi{V} \in \S\carp{V},\v \in V, \vv \in \cl{\v}, \s \in \S \te{ and } \SS\subseteq \S\]
...
\[\sup_{\v\in V} \sup_{\substack{\sv\vsi{V},\alt{\sv}\vsi{V} \in \S\carp{V}, \s\in \S\\ \IGr\vind{\v}(\sv\vsi{V},\s) \neq \IGr\vind{\v}(\alt{\sv}\vsi{V},\s)}} \frac{|\IGr\vind{\v}(\sv\vsi{V},\s) - \IGr\vind{\v}(\alt{\sv}\vsi{V},\s)|}{\sum_{\vv \in \cl{\v}} |\sv\cind{\vv}\vsi{V} - \alt{\sv}\cind{\vv}\vsi{V}|} \leq \const\indx{}\]

So,

\[|\IGr\vind{\v}(\sv\vsi{V},\s) - \IGr\vind{\v}(\alt{\sv}\vsi{V},\s)|\leq \const\indx{}\sum_{\vv \in \cl{\v}} |\sv\cind{\vv}\vsi{V} - \alt{\sv}\cind{\vv}\vsi{V}| \te{ for all } \sv\vsi{V},\alt{\sv}\vsi{V}\in \S\carp{V},\s\in \S\te{ and }\v\in V.\]
...
&=\ex{\sup_{\ttt\leq \t} \left|\int_\S \int_0^{\const\indx{}} \int_0^{\ttt} \s\left(\mb{I}_{\r < \IGr\vind{\v}(\X\tp{\tt-},\s)} - \mb{I}_{\r < \IGr\vind{\v}(\alt{\X}\tp{\tt-},\s)}\right)\,\poiss\vind{\v}(d\tt\,d\r\,d\s)\right|}\\
&\leq \ex{\int_\S\int_0^{\const\indx{}}\int_0^\t \left|\s\left(\mb{I}_{\r < \IGr\vind{\v}(\X\tp{\tt-},\s)} - \mb{I}_{\r < \IGr\vind{\v}(\alt{\X}\tp{\tt-},\s)}\right)\right|\,\poiss\vind{\v}(d\tt\,d\r\,d\s)}\\
&=\int_\S\int_0^\t\int_0^{\const\indx{}} |\s|\ex{\mb{I}_{\IGr\vind{\v}(\X\tp{\tt-},\s) \leq \r < \IGr\vind{\v}(\alt{\X}\tp{\tt-},\s)} + \mb{I}_{\IGr\vind{\v}(\alt{\X}\tp{\tt-},\s) \leq \r < \IGr\vind{\v}(\X\tp{\tt-},\s)}}\,d\r\,d\tt\,\Sm(d\s)\\
...
&\leq \const\indx{}|\S|\int_0^\t \sum_{\vv \in \cl{\v}}\ex{\|\X\cind{\vv} - \alt{\X}\cind{\vv}\|_\tt}\,d\tt\\
\end{align*}
...
\[\sup_{\v \in V}\ex{\|\X\sln{1}\cind{\v} - \X\sln{0}\cind{\v}\|_\t} \leq \t\const\indx{2} \defeq \t\const\indx{}|\S|\]
...
&\leq \sum_{\v \in V\sln{n}}\sum_{\s \in \S} \left|\IGr\vind{\v}(\proj\vsi{V\sln{n}}(\sv\vsi{V}),\s) - \IGr\vind{\v}(\sv\vsi{V},\s)\right|\delt\deltf{\f}\vind{\v} + \sum_{\v \in V\setminus V\sln{n}} \const\indx{}\delt\deltf{\f}\vind{\v}\\
...
\begin{thms}
Let \(\X\) be a Feller process satisfying Assumption \ref{subsec::Assum:Assu::Local} with infinitesimal generator \(\IG\). Let \(\m\sln{} = \law(\X)\). Let \(\root\) be the root of \(G\) and let \(\{1,\dots,\degr\}\) be the children of \(\root\) in \(G\). For any \(\sset \supseteq\sset\vsi{\tree\sln{1}}\ts{\T}\), \(\t\in (0,\T]\) and \(\mmm\sln{} \in \pmsr(\sset)\), define \(\cm\ts{\t}:\sset\vsi{\{\root,1\}}\ts{\t-}\times \pmsr(\sset) \ra \pmsr(\sset\vsi{\{2,\dots,\degr\}}\ts{\t-})\) by,

\begin{equation}
\cm\ts{\t}(\x\cind{\{\root,1\}}\tip{[0,\t)},\mmm\sln{})(\cdot) = \pr\me{\mmm\sln{}}\left(\X\cind{\{2,\dots,\degr\}}\tip{[0,\t)} \in \cdot|\X\cind{\{\root,1\}}\tip{[0,\t)} = \x\cind{\{\root,1\}}\tip{[0,\t)}\right).
\label{sec::Main:eqn::gamma}
\end{equation}
\labe{sec::Main:eqn::gamma}

Then there exists a unique solution, \(\alt{\m\sln{}} \in \pmsr(\sset\vsi{\tree\sln{1}}\ts{\T})\), to the following equation:
...
\[\crate{}(\X\cind{\{\v,\root\}}\tip{[0,\t)},\s) = \exmu{\cm\ts{\t}(\X\cind{\{\v,\root\}}\tip{[0,\t)},\alt{\m\sln{}})}{\IGr\vind{\root}(\X\tp{\t-},\s)}\]

for all \(\s\in \S\). Furthermore, \(\alt{\m\sln{}} = \proj\vsi{\tree\sln{1}}\ts{\T}(\m\sln{})\).
...
\[|\IGr\vind{\v}(\sv\cind{V},\s) - \IGr\vind{\v}(\alt{\sv}\cind{V},\s)| \leq \const\indx{}\sum_{\vv\in \cl{\v}} |\sv\cind{V}\vsi{\vv} - \alt{\sv}\cind{V}\vsi{\vv}|\te{ for all } \sv\cind{V},\alt{\sv}\cind{V}\in \S\carp{V},\s \in \S \te{ and } \v \in V.\]
...
&\ind \os{\mb{I}_{\evnt\ts{\t}}\|\alt{\X}\cind{\neigh\vind{\U}} - \X\cind{\neigh\vind{\U}}\|_\t = 0}{\leq} \left(\int_\S|\s|\,\Sm(d\s)\right)\const\indx{} \int_0^\t \sum_{\vv\in \cl{\v}\cap\U}\ex{\mb{I}_{\evnt\ts{\t}}\|\alt{\X}\cind{\vv} - \X\cind{\vv}\|_\tt}\,d\tt\\
&\ind \os{\evnt\ts{\tt} \supseteq \evnt\ts{\t}}{\leq} |\S|\const\indx{}(\degr+1) \int_0^\t \sup_{\v\in \U}\ex{\mb{I}_{\evnt\ts{\tt}}\|\alt{\X}\cind{\v} - \X\cind{\v}\|_\tt}\,d\tt.
\end{align*}

We can conclude that

\[\sup_{\v\in \U}\ex{\left\|\alt{\X}\cind{\v} - \X\cind{\v}\right\|_{\t}\mb{I}_{\t <\rt\indx{i+1}}} \leq |\S|\const\indx{}(\degr+1)\int_0^\t\sup_{v\in\U}\ex{\left\|\alt{\X}\cind{\v} - \X\cind{\v}\right\|_\tt\mb{I}_{\tt < \rt\indx{i+1}}}\,d\tt.\]
...
\[\pmap(\X)(\{(\rt\indx{},\mark)\}) = \begin{cases}
1 &\te{ if } \X\tp{\rt\indx{}} - \X\tp{\rt\indx{}-} = \mark\\
0 &\te{ otherwise}
\end{cases}.\]
...
\[\pmap\vind{\v}(\X)(\{(\rt\indx{},\mark)\}) = \begin{cases}
...
\[\pmap\vind{\v}(\X)(\{(\rt\indx{},\mark): \X\cind{\v}\tp{\rt\indx{}} - \X\cind{\v}\tp{\rt\indx{}-} \neq \mark\}) = 0.\]
...
\begin{lem}
Suppose Assumption \ref{subsec::ProofE:Assu::Eassu} holds. Let \(\m\sln{} = \law(\X)\). Suppose,

\[\crate\vind{\v}\ts{\t}(\s) := \crate(\X\cind{\{\v,\root\}}\tip{[0,\t)},\s) = \exmu{\cm\ts{\t}(\X\cind{\{\v,\root\}}\tip{[0,\t)},\m\sln{})}{\IGrg(\X\cind{\tree\sln{1}}\tp{\t-},\s)}.\]
...
By assumption there exists a constant \(\const\indx{} < \infty\) such that \(\ex{\int_0^\T \mb{I}_{\evnt}\,d\X} \leq \const\indx{}\) for all elementary \(\evnt\). Taking an expectation of equations \eqref{subsec::ProofE:eqn::upcrossingineq} and \eqref{subsec::ProofE:eqn::bdd}, we get:

\begin{align}
(b-a)\ex{\upcrs\gidd{\Tset}[a,b]} &\leq \const\indx{} + \ex{\max\{a-\X\tp{\T},0\}}
\label{subsec::ProofE:eqn::expupcrossingineq}\\
\pr\left(\sup_{\t\in\Tset} |\X\tp{\t}| > M\right) &\leq \frac{2}{M}\left(\ex{|\X\tp{\T}|} + \const\indx{}\right)
...
\begin{proof}
Fix \(\v\in V\). By Assumption \ref{subsec::ProofE:Assu::Eassu}, \(\pmap(\X\cind{\{\v,\root\}})\) has intensity bounded from above by \(2\const\indx{}\). Therefore, for every \(\t > 0\) there almost surely exists an \(\tt\) small enough such that \(\evnt^{\{\v,\root\}}_{\tt,\t}\) holds. Fix \(\tt < \t\). Let \(\const\indx{}\) be the upper bound of \(\rate\). Fix any \(\s\in \S\). We begin by computing
...
Because the jump rate of \(\pmap\vind{\v}(\X)\) is bounded from above by \(\const\indx{}\) for any \(\v\in\tree\sln{1}\),  when \(\sv\vsi{\tree\sln{1}}\neq\alt{\sv}\vsi{\tree\sln{1}}\), 

\[\pr\left(\X\cind{\tree\sln{1}}\tp{\t-} = \sv\vsi{\tree\sln{1}}|\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\alt{\sv}\vsi{\tree\sln{1}},\alt{\evnt}\ts{\t}\right) \leq 1-e^{-\tt(\degr+1)\const\indx{}}\]

and when \(\sv\vsi{\tree\sln{1}}=\alt{\sv}\vsi{\tree\sln{1}}\),

\[\pr\left(\X\cind{\tree\sln{1}}\tp{\t-} = \sv\vsi{\tree\sln{1}}|\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\alt{\sv}\vsi{\tree\sln{1}},\alt{\evnt}\ts{\t}\right) \geq e^{-\tt(\degr+1)\const\indx{}}.\]

So,

\begin{equation}
e^{-\tt(\degr+1)\const\indx{}}\sum_{\sv\vsi{\tree\sln{1}}\in \S\carp{\tree\sln{1}}} \rate(\sv\vsi{\tree\sln{1}},\s)\pr\left(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t}\right) \leq \crate\vind{\v}\ts{\t}(\x,\s),
...
&\leq \const\indx{}(1 - e^{-\tt(\degr+1)\const\indx{}})|\S|^{\degr(\degr+1)/2} + \sum_{\sv\vsi{\tree\sln{1}} \in \S\carp{\tree\sln{1}}} \rate(\sv\vsi{\tree\sln{1}},\s)\pr\left(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t}\right)
...
\begin{align}
e^{-2\tt\const\indx{}}\pr\left(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t-\tt}\right) &\leq \pr\left(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t}\right)\nonumber\\
&\leq e^{2\tt\const\indx{}}\pr\left(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t-\tt}\right).
...
\[e^{-2\tt\const\indx{}}\crate\vind{\v}\ts{\t-\tt}(\x\cind{\tree\sln{1}},\s) \leq \sum_{\sv\vsi{\tree\sln{1}}\in \S\carp{\tree\sln{1}}} \lambda(\sv\vsi{\tree\sln{1}},\s)\pr(\X\cind{\tree\sln{1}}\tp{(\t-\tt)-} = \sv\vsi{\tree\sln{1}}|\alt{\evnt}\ts{\t})\leq e^{2\tt\const\indx{}}\crate\vind{\v}\ts{\t-\tt}(\x\cind{\tree\sln{1}},\s)\]

Additionally, we can apply equations \eqref{subsec::ProofE:eqn::partlowbd} and  \eqref{subsec::ProofE:eqn::partupbd} to get,

\[e^{-\tt(\degr+3)\const\indx{}}\crate\vind{\v}\ts{\t-\tt}(\x\cind{\tree\sln{1}},\s) \leq \crate\vind{\v}\ts{\t}(\x\cind{\tree\sln{1}},\s) \leq \const\indx{}(1 - e^{-\tt(\degr+1)\const\indx{}})|\S|^{\degr(\degr+1)/2} + e^{2\tt\const\indx{}}\crate\vind{\v}\ts{\t-\tt}(\x\cind{\tree\sln{1}},\s)\]
...
&\hspace{24pt}\leq \max\left\{\crate\vind{\v}\ts{\t-\tt}(\x,\s)\left(1 - e^{-\tt(\degr+3)\const\indx{}}\right),\const\indx{}(1-e^{-\tt(\degr+1)\const\indx{}})|\S|^{\degr(\degr+1)/2} + \crate\vind{\v}\ts{\t-\tt}(\x,\s)\left(e^{2\tt\const\indx{}} - 1\right)\right\}\\
...
\begin{align*}
\ex{|\crate\vind{\v}\ts{\t}(\x,\s) - \crate\vind{\v}\ts{\t-\tt}(\x,\s)|} &\leq \const\indx{}\pr\left(\te{not }\evnt^{\{\v,\root\}}_{\tt,\t}\right) + \ex{\mb{I}_{\evnt^{\{\v,\root\}}_{\tt,\t}}|\crate\vind{\v}\ts{\t}(\x,\s) - \crate\vind{\v}\ts{\t-\tt}(\x,\s)|}\\
...
\[\mb{I}_{\{\X\cind{\{\v,\root\}}\te{ has }n\te{ jumps in } [\t-\tt,\t)\}}|\crate\vind{\v}\ts{\t}(\s) - \crate\vind{\v}\ts{\t-\tt}(\s)|\leq C_1\tt + n \const\indx{}.\]
...
Therefore it suffices to derive the \(\F\vsi{\tree\sln{1}}\ts{\t}\)-intensity of \(\rp\). \(\rp\) has \(\F\vsi{V}\ts{\t}\)-intensity \(\rate\pra{\rp}(\X\tp{\t-},\mark) := \sum_{\v \in \tree\sln{1}}\sum_{\s\in \S}\mb{I}_{\mark = \s\ev\vind{\v}}\IGr\vind{\v}(\X\tp{\t-},\s)\). Each \(\IGr\vind{\v}\) is clearly left-continuous in time, so \(\rate\pra{\rp}\) is also left-continuous in time. Recall that \(\m\sln{}\) is the law of \(\X\). Then,

\[\exmu{\m\sln{}}{\rate\pra{\rp}(\X\tp{\t-},\mark)|\F\vsi{\tree\sln{1}}\ts{\t-}} = \sum_{\v \in \tree\sln{1}}\sum_{\s\in \S} \mb{I}_{\mark = \s\ev\vind{\v}}\exmu{\m\sln{}}{\IGr\vind{\v}(\X\tp{\t-},\s)|\F\vsi{\tree\sln{1}}\ts{\t-}}.\]

Applying Theorem \ref{sec::Main:Thm::CI} and symmetry of \(\m\sln{}\) yields,

\begin{align*}
\exmu{\m\sln{}}{\IGr\vind{\v}(\X\tp{\t-},\s)|\F\vsi{\tree\sln{1}}\ts{\t-}} &= \exmu{\m\sln{}}{\IGr\vind{\v}(\X\tp{\t-},\s)|\X\cind{\tree\sln{1}}\tip{[0,\t)}}\\
&\os{\te{Thm \ref{sec::Main:Thm::CI}}}{=} \exmu{\m\sln{}}{\IGrg{}(\X\cind{\cl{\v}}\tp{\t-},\s)|\X\cind{\{\v,\root\}}\tip{[0,\t)}} \\
&\os{\te{Sym}}{=} \exmu{\XX\sim \m\sln{}}{\IGrg{}(\XX\cind{\tree\sln{1}}\tp{\t-},\s)|\XX\cind{\{\root,1\}}\tip{[0,\t)} = \X\cind{\{\v,\root\}}\tip{[0,\t)}}\\
&= \exmu{\XX\sim \proj\vsi{\tree\sln{1}}\ts{\T}(\m\sln{})}{\IGrg{}(\XX\cind{\tree\sln{1}}\tp{\t-},\s)|\XX\cind{\{\root,1\}}\tip{[0,\t)} = \X\cind{\{\v,\root\}}\tip{[0,\t)}}\\
&= \exmu{\cm\ts{\t}\left(\X\cind{\{\v,\root\}}\tip{[0,\t)},\proj\vsi{\tree\sln{1}}\ts{\T}(\m\sln{})\right)}{\IGrg{}(\X\cind{\tree\sln{1}}\tp{\t-},\s)}\\
...
\ind By lemma \ref{subsec::Some:Lem::PPtoSDE}, we can characterize the law of \(\proj\vsi{\tree\sln{1}}\ts{\T}(\X)\) as the law of the unique solution (given \(\proj\vsi{\tree\sln{1}}\ts{\T}(\m\sln{})\)) to the SDE
...
\ind We will prove uniqueness of the local SDE by showing that \(\mmm\sln{1}\) is the marginal distribution of \(\m\sln{}\), which is the law of \(\X\) (as defined in equation \eqref{subsec::Notat:Eqn::IG}). To do that, we will construct a sequence of random variables \(\{\X\sln{n}:n\in\mb{N}\}\) on \(\sset\vsi{\tree\sln{n}}\ts{\T-}\) converging weakly to \(\X\). 
...
\purpose By constructing larger processes with the whose marginal distribution is \(\mmm\sln{1}\), we can, in the limit, construct an interacting particle system on \(V\) that projects down to \(\mmm\sln{}\). All that will remain will be to prove that this process is equal in distribution to \(\m\sln{}\).
...
f&\te{ is a }[0,\const\indx{}] \te{-valued mapping and } \pmap\vind{\v}(\XXX) = \{(\rt\indx{\it}\vind{\v},\mark\indx{\it}\vind{\v}):\it\in\mb{N}\}.\nonumber
\end{align} 
...
\[\cdense\ts{\t}(\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}};\x\cind{\{\root,1\}}) = \frac{\dense\sln{1}\ts{\t}([\X\sln{1}\cind{\tree\sln{1}\setminus\{\root\}},\x\cind{\{\root,1\}}])}{\alt{\dense\sln{}\ts{\t}}(\x\cind{\{1,\root\}})}.\]
...
For any \(\U\subseteq V\), we can consider the embedding \(\pmsr\left(\sset\vsi{\U}\ts{\t-}\right) = \pmsr\left((\sset\vsi{\U}\ts{\t-},\F\vsi{\U}\ts{\t-})\right) \subseteq \pmsr\left((\sset\vsi{V}\ts{\t-},\F\vsi{\U}\ts{\t-})\right)\). That is, a measure \(\mmm\sln{}\in \pmsr\left(\sset\vsi{\U}\ts{\t-}\right)\) can be considered as a \(\F\vsi{\U}\)-adapted measure on \(\sset\vsi{V}\ts{\t-}\).
...
Therefore the conditions of the Daniell-Kolmogorov extension theorem are satisfied. Therefore, there exists a \(\mmm\sln{}\in \ms{P}(\sset\vsi{\U}\ts{\T-})\) such that \(\proj\vsi{\U}\ts{\T-}(\mmm\sln{}) =\mmm\vind{\U}\) for all finite \(\U\). \tr{citation needed (Reading off of Lecture notes (Theorem 2.4.3) which were used as the basis of Terence Tao's measure theory book)}.

\ind In particular, there exists a probability space with canonical process \(\X\) such that \(\mmm\sln{} = \law(\X\tip{[0,\T)})\) and \(\mmm\vind{\v} = \law(\X\cind{\v}\tip{[0,\T)})\) for all \(\v \in V\). In particular, this means that \(\law(\X\cind{\tree\sln{n}}\tip{[0,\T)}) = \mmm\vind{\tree\sln{n}} = \mmm\sln{n}\). Since \(\X\cind{\tree\sln{n}} \ra \X\) almost surely (we are in the topology of componentwise convergence), \(\mmm\sln{n} \Rightarrow \mmm\sln{}\). In order to characterize \(\X\) and \(\mmm\sln{}\), we construct a stochastic differential equation on a probability space on which \(\X\sln{n} \sim \mmm\sln{n}\) and \(\X\sln{n} = \X\cind{\tree\sln{n}}\) almost surely. \tr{I don'\t know if it's clear, but we are taking \(\X\sln{n}\) and \(\X\) as given in this probability space.}
...
\X\sln{}\cind{\v}\tp{0} + \int_{(0,\t]}\int_\S\int_{(0,\crate(\X\sln{n}\cind{\{\v,\p\vind{\v}\}}\tip{[0,\tt)},\s)]}\s\alt{\poiss}\vind{\v}(d\tt,d\s,d\r)\te{ for } \v \in \tree\sln{n}\setminus\tree\sln{n-1}\\
...
However, by Propositions \ref{subsec::Well-:Prop::SDEWP} and \ref{subsec::Well-:Prop::SDE=IG}, equation \eqref{subsec::ProofU:eqn::XSDE} has a unique solution, and that solution has distribution \(\m\sln{}\). Thus, \(\mmm\sln{} = \m\sln{}\). We conclude that \(\mmm\sln{1} = \proj\vsi{\tree\sln{1}}\ts{\T-}(\m\sln{})\).
...
\begin{lem}
Assume the notation and assumptions from Lemmas \ref{subsec::Some:Lem::genfiniteSDE} and \ref{subsec::Some:Lem::PPtoSDE} hold under the probability measure \(\m\sln{}\). Suppose under \(\mm\), \(\X\) solves,
...
\begin{equation}
\frac{d\m\sln{}\ts{\t}}{d\mm\ts{\t}}= \frac{d\m\sln{}\ts{0}}{d\mm\ts{0}}\left(\prod_{0< \rt\indx{\it}\leq \t} \ratee\ts{\t}(\mark\indx{\it})\right)\exp\left(-\int_{(0,\t]\times \S} (\ratee\ts{\tt}(\s) - 1)\,d\tt\alt{\Sm}(d\s)\right)
...
\ind By Lemma \ref{subsec::Some:Lem::PPtoSDE}, \(\rp\) has predictable intensity \(\ratee\ts{\t}(\cdot)\) with respect to \(\F\ts{\t}\) and \(\m\sln{}\). \(\rp\) has predictable intensity 1 with respect to \(\F\ts{\t}\) and \(\mm\). This holds all of the information about the original process \(\X\), so we can consider \(\frac{d\m\sln{}\ts{\t}}{d\mm\ts{\t}}\) as a change of measure affecting \(\rp\) instead of \(\X\). In Lemma \ref{subsec::Some:Lem::genfiniteSDE}, it was assumed that \(\ratee\ts{\t}\vind{\v}\) is bounded. Thus, the result follows directly from Proposition \ref{subsec::Prope:Prop::radnikder}. 
\end{proof}
...
\[\sup_{n}\sup_{\sv\vsi{V}\in \S\carp{V}}\sup_{\v\in V}\sup_{\s\in \S} \IGr\slnvind{n}{\v}(\sv\vsi{V},\s) \leq \const\indx{},\]
...
\[\XX\tp{\t} = \sum_{\v \in \U}\cconst\ev\vind{\v}\int_0^\t\int_0^{\const\indx{}}\alt{\poiss}\vind{\v}(d\r, d\tt)\]
