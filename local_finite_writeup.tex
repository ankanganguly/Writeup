...
\[\sup_{\v\in V} \sup_{\substack{\sv\cind{}\vsi{V},\alt{\sv}{}{V} \in \S\carp{V}, \s\in \S\\ \IGr\vind{\v}(\sv\cind{}\vsi{V},\s) \neq \IGr\vind{\v}(\alt{\sv}{}{V},\s)}} \frac{|\IGr\vind{\v}(\sv\cind{}\vsi{V},\s) - \IGr\vind{\v}(\alt{\sv}{}{V},\s)|}{\sum_{\vv \in \cl{\v}} |\sv\cind{\vv}\vsi{V} - \alt{\sv}{\vv}{V}|} \leq \const\indx{}\]

So,

\[|\IGr\vind{\v}(\sv\cind{}\vsi{V},\s) - \IGr\vind{\v}(\alt{\sv}{}{V},\s)|\leq \const\indx{}\sum_{\vv \in \cl{\v}} |\sv\cind{\vv}\vsi{V} - \alt{\sv}{\vv}{V}| \te{ for all } \sv\cind{}\vsi{V},\alt{\sv}{}{V}\in \S\carp{V},\s\in \S\te{ and }\v\in V.\]
...
\[\sup_{\v \in V} \ex{\|\X\cind{\v}\tip{}{n+1} - \X\cind{\v}\tip{}{n}\|_\t} \leq (\degr+1)\const\indx{1} \int_0^\t \sup_{\v \in V} \ex{\|\X\cind{\v}\tip{}{n} - \X\cind{\v}\tip{}{n-1}\|_\tt}\,d\tt\] 
...
\[\sup_{\v \in V}\ex{\|\X\cind{\v}\tip{}{1} - \X\cind{\v}\tip{}{0}\|_\t} \leq \t\const\indx{2} \defeq \t\const\indx{}|\S|\]

Iterating yields

\[\sup_{\v \in V} \ex{\|\X\cind{\v}\tip{}{n+1} - \X\cind{\v}\tip{}{n}\|_\t} \leq \t\const\indx{2}\frac{(\t(\degr+1)\const\indx{1})^n}{(n+1)!}\]

Then,

\begin{align*}
\sum_{n=0}^\infty \sup_{\v \in V} \ex{\|\X\cind{\v}\tip{}{n+1} - \X\cind{\v}\tip{}{n}\|_\T} &= \sum_{n=1}^\infty \frac{\const\indx{2}}{(\degr+1)\const\indx{1}}\frac{(\T(\degr+1)\const\indx{1})^{n}}{n !}\\
...
\begin{align*}
\sup_{\v \in V} \ex{\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}{n+1}\|_\T} &\leq (\degr+1)\const\indx{1}\int_0^\T \sup_{\v \in V}\ex{\|\X\cind{\v}\tip{} - \X\cind{\v}\tip{}{n}\|_\tt}\,d\tt \\
&\leq \T\const\indx{1}(\degr+1)\sup_{\v\in V} \ex{\|\X\cind{\v}\tip{} - \X\cind{\v}\tip{}{n}\|_\T}.
\end{align*}

Taking a limit as \(n\) goes to infinity on both sides yields

\[\lim_{n\ra\infty} \sup_{\v \in V} \ex{\|\alt{\X}\cind{\v}\tip{} - \X\cind{\v}\tip{}{n}\|_\T} = 0.\]
...
\[\alt{\X}\cind{\v}\tp{\cdot} = \X\cind{\v}\tp{\t}{\sv\cind{}\vsi{V}} + \int_\S \int_0^\cdot \s\,\poiss\vind{\v}(d\tt,(0,\IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)],d\s).\]

Then \(\X\cind{}\tp{\t+\cdot}{\sv\cind{}\vsi{V}} \deq \alt{\X}\cind{}\tp{\cdot}\). By the definition of \(\X\cind{}\tip{}{\sv\cind{}\vsi{V}}\), \(\alt{\X}\cind{}\tip{} \deq \X\cind{}\tip{}{\X\cind{}\tp{\t}{\sv\cind{}\vsi{V}}}\) as desired.
...
\alt{\X}\cind{\v}\tp{\t} &= \alt{\sv}{\v}{V} + \int_\S\int_0^\t \s\,\poiss\vind{\v}(d\tt\,(0,\IGr\vind{\v}(\alt{\X}\cind{}\tp{\tt-},\s)],d\s)&\te{ for all } \v \in V\\
\end{align*}

Then by Lemma \ref{subsec::Well-:Lem::SDEWP},

\[\ex{\|\X\cind{\v}\tip{} - \alt{\X}\cind{\v}\tip{}\|_\t} \leq |\sv\cind{\v}\vsi{V} - \alt{\sv}{\v}{V}| + \const\indx{1}\int_0^\t \sum_{\vv\in \cl{\v}} \ex{\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\]
...
&\leq \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \sum_{\v \in V}\const\indx{1}\int_0^\t \sum_{\vv \in \cl{\v}} \ex{\b\cind{\v}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&\os{\b\cind{\v}\leq \b(\b\cind{\vv})}{\leq} \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \sum_{\v \in V}\const\indx{1}\int_0^\t \sum_{\vv \in \cl{\v}} \b\ex{\b\cind{\vv}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&= \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \b\const\indx{1}\int_0^\t \sum_{\v \in V}\sum_{\vv \in \cl{\v}} \ex{\b\cind{\vv}\|\X\cind{\vv}\tip{} - \alt{\X}\cind{\vv}\tip{}\|_\tt}\,d\tt\\
&=\|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b} + \b(\degr+1)\const\indx{1}\int_0^\t \ex{\|\X\cind{}\tip{} - \alt{\X}\cind{}\tip{}\|_{\b,\tt}}\,d\tt\\
\end{align*}

Applying Gronwall's inequality:

\[\ex{\|\X\cind{}\tip{} - \alt{\X}\cind{}\tip{}\|_{\b,\T}} \leq \|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b}e^{\T\b(\degr+1)\const\indx{1}}\]
...
&\leq \psi\left(\|\sv\cind{}\vsi{V} - \alt{\sv}{}{V}\|_{\b}e^{\T\b(\degr+1)\const\indx{1}}\right) \os{\sv\cind{}\vsi{V} \ra\alt{\sv}{}{V}}{\ra} \ep.
\end{align*}
...
\[|\IGr\vind{\v}(\sv\cind{V}\vsi{},\s) - \IGr\vind{\v}(\alt{\sv}{V}{},\s)| \leq \const\indx{}\sum_{\vv\in \cl{\v}} |\sv\cind{V}\vsi{\vv} - \alt{\sv}{V}{\vv}|\te{ for all } \sv\cind{V}\vsi{},\alt{\sv}{V}{}\in \S\carp{V},\s \in \S \te{ and } \v \in V.\]
...
\[\pr\left(\X\cind{\tree\sln{1}}\tp{\t-} = \sv\cind{}\vsi{\tree\sln{1}}|\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\alt{\sv}{}{\tree\sln{1}},\alt{\evnt}{	}\right) \leq 1-e^{-\tt(\degr+1)\const\indx{}}\]

and when \(\sv\cind{}\vsi{\tree\sln{1}}=\alt{\sv}{}{\tree\sln{1}}\),

\[\pr\left(\X\cind{\tree\sln{1}}\tp{\t-} = \sv\cind{}\vsi{\tree\sln{1}}|\X\cind{\tree\sln{1}}\tp{(\t-\tt)-}=\alt{\sv}{}{\tree\sln{1}},\alt{\evnt}{	}\right) \geq e^{-\tt(\degr+1)\const\indx{}}.\]
...
&\leq \sum_{\it=1}^n \const\indx{1}|\t{\it} - \tt{\it}| + \const\indx{}\ex{\te{\# of jumps of }\X\cind{\{\v,\root\}}\tip{[\tt{\it},\t{\it})}}\\
...
\[\sup_{n}\sup_{\sv\cind{}\vsi{V}\in \S\carp{V}}\sup_{\v\in V}\sup_{\s\in \S} \IGr\vindsln{n}{\v}(\sv\cind{}\vsi{V},\s) \leq \const\indx{},\]
