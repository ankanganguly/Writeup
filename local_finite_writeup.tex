...
\begin{equation}
\IG f(\sv\cind{}\vsi{V}) = \sum_{\v \in V}\sum_{\s \in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V} + \s \ev{\v}) - f(\sv\cind{}\vsi{V})],
...
\begin{equation}
\delt\deltf{\f}{\v} \defeq \sup\left\{|f(\sv\cind{}\vsi{V}) - f(\sv\cind{}\vsi{V}+\s\ev{\v})|: \sv\cind{}\vsi{V} \in \S\carp{V},\s \in \S\right\}.
...
\[\sup_{\v\in V}\sum_{\vv\neq \v} \sup_{\substack{\sv\cind{}\vsi{V} \in \S\carp{V}\\ \s\in \S\\ \SS \subseteq \S}} \left|\sum_{\ss \in \SS} (\IGr{\v}(\sv\cind{}\vsi{V},\ss) - \IGr{\v}(\sv\cind{}\vsi{V}+\s \ev{\vv},\ss))\right| < \infty.\]
\end{enumerate}
...
\[\sup_{\substack{\sv\cind{}\vsi{V} \in \S\carp{V}\\ \s\in \S\\ \SS\subseteq \S}} \left|\sum_{\ss \in \SS} (\IGr{\v}(\sv\cind{}\vsi{V},\ss) - \IGr{\v}(\sv\cind{}\vsi{V}+\s \ev{\vv},\ss))\right| = 0 \te{ for all } \vv \notin \cl{\v}\]
...
\[\left|\sum_{\ss \in \SS} (\IGr{\v}(\sv\cind{}\vsi{V},\ss) - \IGr{\v}(\sv\cind{}\vsi{V}+\s\ev{\vv},\ss))\right| \leq \const{}|\S| \te{ for all } \sv\cind{}\vsi{V} \in \S\carp{V},\v \in V, \vv \in \cl{\v}, \s \in \S \te{ and } \SS\subseteq \S\]

Finally, by statement 3 of Assumption \ref{subsec::Assum:Assu::CI}, \(|\neigh\vind{\v}| \leq \degr\) for all \(\v \in V\). Thus,

\begin{align*}
\sup_{\v\in V}\sum_{\vv \neq \v} \sup_{\substack{\sv\cind{}\vsi{V} \in \S\carp{V}\\ \s\in \S\\ \SS\subseteq \S}}& \left|\sum_{\ss \in \SS} (\IGr{\v}(\sv\cind{}\vsi{V},\ss) - \IGr{\v}(\sv\cind{}\vsi{V}+\s\ev{\vv},\ss))\right|\\
&  = \sup_{\v\in V}\sum_{\vv\in \neigh\vind{\v}} \sup_{\substack{\sv\cind{}\vsi{V} \in \S\carp{V}\\ \s\in \S\\ \SS\subseteq \S}} \left|\sum_{\ss \in \SS} (\IGr{\v}(\sv\cind{}\vsi{V},\ss) - \IGr{\v}(\sv\cind{}\vsi{V}+\s\ev{\vv},\ss))\right|\\
...
\[\IG f(\sv\cind{}\vsi{V}) = \sum_{\v\in V}\sum_{\s \in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V} + \ev{\v}\s) - f(\sv\cind{}\vsi{V})].\]
...
&= \lim_{\t \searrow 0} \ex{\sum_{\v \in V} \frac{1}{\t}\int_\S\int_0^\t \left[f(\X{}{\tt-} + \s\ev{\v}) - f(\X{}{\tt-})\right]\,\poiss{\v}\left(d\tt,(0,\IGr{\v}(\X{}{\tt-},\s)],d\s\right)\middle|\X{}{0} = \sv\cind{}\vsi{V}}\\
&\os{f \in \mc{D}}{=} \lim_{\t \searrow 0}\sum_{\v \in V} \frac{1}{\t}\int_\S\int_0^\t \ex{\left[f(\X{}{\tt-} + \s\ev{\v}) - f(\X{}{\tt-})\right]\IGr{\v}(\X{}{\tt-},\s)\middle|\X{}{0} = \sv\cind{}\vsi{V}}d\tt\,\Sm(d\s)\\
&\os{\te{Lebesgue Differentiation}}{=} \sum_{\v \in V} \int_\S \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V} + \s\ev{\v}) - f(\sv\cind{}\vsi{V})]\,\Sm(d\s)\\
&= \sum_{\v \in V} \sum_{\s \in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V} + \s\ev{\v}) - f(\sv\cind{}\vsi{V})]
\end{align*}
\end{proof}
...
\begin{equation}
\IG\sln{n}f(\sv\cind{}\vsi{V\sln{n}}) = \sum_{\v\in V\sln{n}}\sum_{\s\in \S} \IGr{\v}(\sv\cind{}\vsi{V\sln{n}}, \s)[f(\sv\cind{}\vsi{V\sln{n}} + \s\ev{\v}) - f(\sv\cind{}\vsi{V\sln{n}})]\te{ for all } \sv\cind{}\vsi{V\sln{n}} \in \S\carp{V\sln{n}}.
...
&= \left|\sum_{\v \in V\sln{n}} \sum_{\s \in \S} \IGr{\v}(\proj{V\sln{n}}{}(\sv\cind{}\vsi{V}),\s)[f(\sv\cind{}\vsi{V}+\s\ev{\v}) - f(\sv\cind{}\vsi{V})] - \sum_{\v \in V}\sum_{\s \in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V}+\s\ev{\v}) - f(\sv\cind{}\vsi{V})]\right|\\
&\leq  \left|\sum_{\v \in V\sln{n}} \sum_{\s \in \S} \IGr{\v}(\proj{V\sln{n}}{}(\sv\cind{}\vsi{V}),\s)[f(\sv\cind{}\vsi{V}+\s\ev{\v}) - f(\sv\cind{}\vsi{V})] - \sum_{\v \in V\sln{n}}\sum_{\s \in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V}+\s\ev{\v}) - f(\sv\cind{}\vsi{V})]\right|\\
&\ind  +\left|\sum_{\v \notin V\sln{n}} \sum_{\s\in \S} \IGr{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V}+\s\ev{\v}) - f(\sv\cind{}\vsi{V})]\right|\\
...
Here \(\delt\deltf{\f}{\v} = \sup\left\{|f(\sv\cind{}\vsi{V}) - f(\sv\cind{}\vsi{V} + \s\ev{\v}): \sv\cind{}\vsi{V}\in \S\carp{V},\s \in \S\right\}\) as defined in equation \eqref{subsec::Well-:Eqn::fvar}. By Corollary \ref{sec::Infin:Cor::Convergence}, \(\X{}{}{n}\Rightarrow \X{}{}\).
\end{proof}
...
\X{\neigh\vind{\U}}{\rt{i}-} + \rv^2\ev{\rv^1} &\te{ if } \rv^3< \rate{\rv^1}(\X{}{\rt{i}-}, \rv^2)\\
...
\[\rate{\proj{\{\v,\root\}}{\T}(\rp{})}{\t}(\X{}{[0,\t)}{1},\mark{}) \defeq \sum_{\s\in \S} \left(\mb{I}_{\mark{} = \s\ev{\v}}\crate{}{}(\X{\{\v,\root\}}{[0,\t)}{1},\s) + \mb{I}_{\mark{} = \s\ev{\root}}\IGr{\root}(\X{}{\t-}{1},\s)\right)\te{ for } \kappa \in \S\carp{\{\v,\root\}}.\]
...
\rate{}(\X{\cl{\v}}{\t-}{n},\s) &\te{ if } \sv\cind{}\vsi{\tree\sln{n}} = \s\ev{\v},\s \in \S\setminus\{0\},\v\in\tree\sln{n-1}\\
\crate{}{}(\X{\{\v,\p{\v}\}}{[0,\t)}{n},\s) &\te{ if } \sv\cind{}\vsi{\tree\sln{n}} = \s\ev{\v},\s\in\S\setminus\{0\},\v\in\tree\sln{n}\setminus\tree\sln{n-1}\\
0 &\te{ otherwise.}
\end{cases}\]
...
Note that for any fixed \(x \in \te{supp}(X)\) and any \(i \in \{1,2\}\), there exists a regular conditional distribution \(\pr{i}_x\) which describes the law of \(\rp\) conditioned on \(X = x\).

\[\mb{E}^2[f(X,\rp[0,\t])] = \int_\spce \mb{E}^2_x[f(x,\rp[0,\t])]\,d\mm^2(x)\]

Note that for any fixed \(x\), \(\pr{i}_x\) and \(\rp\) satisfy the conditions of \cite[Theorem 14.4.I]{DalVer08}. So,
...
\ratee{\t,\v}(\XX{}{[0,\t)},\s) &\te{ if } \sv\cind{}\vsi{V} = \s\ev{\v}\\
0 &\te{ otherwise,}
\end{cases}
...
\ratee{\t,\v}(\X{}{[0,\t)},\sv\cind{}\vsi{V}) &\te{ if } \sv\cind{}\vsi{V} = \s\ev{\v}\\
0&\te{ otherwise.}
\end{cases}\]
...
\[\IG\sln{n}f(\sv\cind{}\vsi{V}) = \sum_{\v\in V}\sum_{\s \in \S} \IGr\sln{n}{\v}(\sv\cind{}\vsi{V},\s)[f(\sv\cind{}\vsi{V} + \s\ev{\v}) - f(\sv\cind{}\vsi{V})].\]
