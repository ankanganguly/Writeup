...
\[e^{-\tt(\degr+3)\const\indx{}}\crate{\v}{\t-\tt}(\x\cind{\tree\sln{1}}\tip{},\s) \leq \crate{\v}{\t}(\x\cind{\tree\sln{1}}\tip{},\s) \leq \const\indx{}(1 - e^{-\tt(\degr+1)\const\indx{}})|\S|^{\degr(\degr+1)/2} + e^{2\tt\const\indx{}}\crate{\v}{\t-\tt}(\x\cind{\tree\sln{1}}\tip{},\s)\]
...
&\hspace{24pt}\leq \max\left\{\crate{\v}{\t-\tt}(\x\cind{}\tip{},\s)\left(1 - e^{-\tt(\degr+3)\const\indx{}}\right),\const\indx{}(1-e^{-\tt(\degr+1)\const\indx{}})|\S|^{\degr(\degr+1)/2} + \crate{\v}{\t-\tt}(\x\cind{}\tip{},\s)\left(e^{2\tt\const\indx{}} - 1\right)\right\}\\
...
\begin{align*}
\ex{|\crate{\v}{\t}(\x\cind{}\tip{},\s) - \crate{\v}{\t-\tt}(\x\cind{}\tip{},\s)|} &\leq \const\indx{}\pr\left(\te{not }\evnt{}^{\{\v,\root\}}_{\tt,\t}\right) + \ex{\mb{I}_{\evnt{}^{\{\v,\root\}}_{\tt,\t}}|\crate{\v}{\t}(\x\cind{}\tip{},\s) - \crate{\v}{\t-\tt}(\x\cind{}\tip{},\s)|}\\
...
where \(\{[\tt{\it},\t\indx{\it}):\it=1,\dots,n\}\) are disjoint, if \(\tt{\it} = \t\indx{\it} = 0\), then \([\tt{\it},\t\indx{\it}) = \{0\}\) and \(\typset\sln{\it}\) are all \(\sigma(\crate{\v}{\t}(\s):\s \in \S,\t\in [0,\tt{\it}])\)-measurable. 

By Lemma \ref{subsec::ProofE:Lem::pleft}, there exists a \(\const\indx{1} < \infty\) such that,

\begin{equation}
\mb{I}_{\evnt{}^{\{\v,\root\}}_{\tt,\t}}|\crate{\v}{\t}(\s) - \crate{\v}{\t-\tt}(\s)| \leq \const\indx{1}\tt.
\label{subsec::ProofE:eqn::driftbound}
\end{equation}
\labe{subsec::ProofE:eqn::driftbound}

Furthermore, since \(\crate{\v}{\t}(\s)\) is a \([0,\const\indx{}]\)-valued process, jumps of \(\crate{\v}{\t}(\s)\) are bounded by \(\const\indx{}\). Thus, for any \(n \in \mb{N}\),

\[\mb{I}_{\{\X\cind{\{\v,\root\}}\tip{}\te{ has }n\te{ jumps in } [\t-\tt,\t)\}}|\crate{\v}{\t}(\s) - \crate{\v}{\t-\tt}(\s)|\leq C_1\tt + n \const\indx{}.\]
...
\begin{align*}
\ex{\int_0^\T \mb{I}_\typset\,d\crate{\v}{\t}(\s)} &= \sum_{\it = 1}^n \mb{I}_{	ypset{\it}}\ex{\left|\crate{\v}{\t\indx{k}}(\s) - \crate{\v}{\tt{\it}}(\s)\right|}\\
...
\[\sup_{\typset\te{ elementary}} \ex{\int_0^\T \mb{I}_\typset\,d\crate{\v}{\t}(\s)} \leq \const\indx{1}\T + 2\const\indx{}^2\T < \infty.\]
\end{proof}
...
\ind We will prove uniqueness of the local SDE by showing that \(\mmm{}{}{1}\) is the marginal distribution of \(\m\sln{}\ts{}\), which is the law of \(\X\cind{}\tip{}\) (as defined in equation \eqref{subsec::Notat:Eqn::IG}). To do that, we will construct a sequence of random variables \(\{\X\sln{n}\cind{}\tip{}:n\in\mb{N}\}\) on \(\sset\vsi{\tree\sln{n}}\ts{\T-}\) converging weakly to \(\X\cind{}\tip{}\). 
...
Let \(\mm\vind{}\ts{} = \law(\XX{}{})\). Suppose \(\mmm{\v}{0,}{1}\ll\mm\vind{}\ts{0}\) for all \(\v\in\tree\sln{1}\). Let \(\{\mm\vind{\v}\ts{}:\v\in V\}\) be the distributions of a sequence of i.i.d. \(\S\carp{\v}\)-valued c\`adl\`ag processes. For any \(\U\subseteq V\), let 
...
\purpose By constructing larger processes with the whose marginal distribution is \(\mmm{}{}{1}\), we can, in the limit, construct an interacting particle system on \(V\) that projects down to \(\mmm{}{}{}\). All that will remain will be to prove that this process is equal in distribution to \(\m\sln{}\ts{}\).
...
\begin{align}
\dense{1}{\t}(\X\cind{\tree\sln{1}}\tip{})&\defeq \frac{d\mmm{}{\t}{1}}{d\mm\sln{1}\ts{\t}}\nonumber\\
&= \frac{d\mmm{}{0}{1}}{d\mm\sln{1}\ts{0,}}\exp\left(\sum_{0< \rt{\it} \leq \t} \log{\ratee{}(\X\sln{1}\cind{}\tip{[0,\rt{\it})},\mark{\it})} - \int{(0,\t]\times\S} [\ratee{}(\X\sln{1}\cind{}\tip{[0,\tt)},\s) - 1]\,d\tt\,\Sm(d\s)\right)\nonumber\\
&= \frac{d\mmm{}{0}{1}}{d\mm\sln{1}\ts{0,}}\exp\Bigg(\sum_{\v\in\tree\sln{1}\setminus\{\root\}}\left(\sum_{0< \rt{\it}_\v \leq \t} \log{\crate{}{}(\X\sln{1}\cind{\{\v,\root\}}\tip{[0,\rt{\it}_\v)},\mark{\it})} - \int{(0,\t]\times\S} [\crate{}{}(\X\sln{1}\cind{\{\v,\root\}}\tip{[0,\tt)},\s) - 1]\,d\tt\,\Sm(d\s)\right)\nonumber\\
...
&= \frac{d\mmm{}{0}{1}}{d\mm\sln{1}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{1}\setminus\{\root\}} \ds{\v}{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{},\crate{}{}) + \ds{\root}{\t}(\X\sln{1}\cind{\tree\sln{1}}\tip{},\IGrg{})\right).
...
\begin{equation}
\alt{\dense}{}{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{}) \defeq \frac{d\mmm{\{\v,\root\}}{\t,}{}}{d\mm\vind{\{\v,\root\}}\ts{\t,}} = \frac{d\mmm{\{\v,\root\}}{0,}{}}{d\mm\vind{\{\v,\root\}}\ts{0,}}\exp\left(\ds{\v}{\t}(\X\sln{1}\cind{\{\v,\root\}}\tip{},\crate{}{}) + \ds{\root}{\t}(\X\sln{1}\cind{\{\root,\v\}}\tip{},\crate{}{})\right).
...
& \defeq\frac{d\mmm{\tree\sln{1}\setminus\{\root,1\}}{0,}{}}{d\mm\vind{\tree\sln{1}\setminus\{\root,1\}}\ts{0,}}\exp\left(\sum_{\v\in \tree\sln{1}\setminus\{\root,1\}} \ds{\v}{\t}([\X\sln{1}\cind{\{v\}}\tip{},\x\cind{\root}\tip{}],\crate{}{}) + \ds{\root}{\t}([\X\sln{1}\cind{\tree\sln{1}\setminus\{\root,1\}}\tip{},\x\cind{\{\root,1\}}\tip{}],\IGrg{}) - \ds{\root}{\t}(\x\cind{\{\root,1\}}\tip{},\crate{}{})\right)\\
\end{align*}
...
\ind Let \(\mm\sln{n}\ts{}\defeq \mm\vind{\tree\sln{n}}\ts{}\) and assume for any \(\U \subseteq V\), \(\mmm{\U}{0,}{} = \otimes_{\v\in\U}\mmm{\v}{0,}{}\) where \(\{\mmm{\v}{0,}{}:\v\in V\}\) describes a sequence of i.i.d. \(\S\)-valued random variables. We can show using induction that,

\begin{equation}
\dense{n}{\t}(\X\sln{n}\cind{}\tip{}) = \frac{d\mmm{}{\t}{n}}{d\mm\sln{n}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{n-1}}\ds{\v}{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n}\setminus\tree\sln{n-1}} \ds{\v}{\t}(\X\sln{n}\cind{\{v,\p{\v}\}}\tip{},\crate{}{})\right).
...
&=\left(\frac{d\mmm{}{0}{n-1}}{d\mm\sln{n-1}\ts{0}}\prod_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}}\frac{d\mmm{\c{\v}}{0,}{}}{d\mm\vind{\c{\v}}\ts{0,}}\right)\exp\Bigg(\sum_{\v\in\tree\sln{n-2}}\ds{\v}{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n-1}\setminus\tree\sln{n-2}} \ds{\v}{\t}(\X\sln{n}\cind{\{\v,\p{\v}\}}\tip{},\crate{}{})\\
...
&=\frac{d\mmm{}{0}{n}}{d\mm\sln{n}\ts{0}}\exp\left(\sum_{\v\in\tree\sln{n-1}}\ds{\v}{\t}(\X\sln{n}\cind{\cl{\v}}\tip{},\IGrg{}) + \sum_{\v\in\tree\sln{n}\setminus\tree\sln{n-1}} \ds{\v}{\t}(\X\sln{n}\cind{\{\v,\p{\v}\}}\tip{},\crate{}{})\right).
\end{align*}
...
\[\frac{d\mmm{}{\t}{n}}{d\mm\sln{n}\ts{\t}} = \dense{n}{\t}(\X\sln{n}\cind{}\tip{}).\]

Then,

\begin{align*}
\frac{d\proj\vsi{\tree\sln{n-1}}\ts{\t}(\mmm{}{\t}{n})}{d\mm\sln{n-1}\ts{\t}} &= \int{\sset\vsi{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t-}} \frac{d\mmm{}{\t}{n}}{d\mm\sln{n}\ts{\t}}\,\mm\vind{\tree\sln{n}\setminus\tree\sln{n-1}}\ts{\t,}(d\X\sln{n}\cind{\tree\sln{n}\setminus\tree\sln{n-1}}\tip{})\\
...
\item \(\frac{d\mmm{}{\t}{n}}{d\mm\sln{n}\ts{\t}} = \dense{n}{\t}(\X\sln{n}\cind{}\tip{})\).
\end{enumerate}
...
However, by Propositions \ref{subsec::Well-:Prop::SDEWP} and \ref{subsec::Well-:Prop::SDE=IG}, equation \eqref{subsec::ProofU:eqn::XSDE} has a unique solution, and that solution has distribution \(\m\sln{}\ts{}\). Thus, \(\mmm{}{}{} = \m\sln{}\ts{}\). We conclude that \(\mmm{}{}{1} = \proj\vsi{\tree\sln{1}}\ts{\T-}(\m\sln{}\ts{})\).
