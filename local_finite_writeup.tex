...
\item Let \(\root \in  V\). Define the sequence of sets \( V\sln{0}\subset  V\sln{1}\subset \cdots \subseteq  V\) as follows: let \( V\sln{0} = \{\root\}\). For \(\numb \geq 1\), let \( V\sln{\numb+1} = \cl{ V\sln{\numb}}\).
...
\begin{proof}
First, for each \(\numb\in\mb{N}\), \(\X{}{}{\numb}\) satisfies Assumption \ref{subsec::Assum:Assu::CI}, so by Proposition \ref{subsec::Well-:Prop::Welldef}, \(\X{}{}{\numb}\) is uniquely defined by its initial condition \(\X{}{0}{\numb}\) and its infinitesimal generator \(\IG\sln{\numb}\).

\ind To prove weak convergence, we first begin by showing that \(\{\X{}{}{\numb}:\numb\in \mb{N}\}\) is tight. Note that \(\S^ V\) is compact, so \(\sup_{\sv{}{ V} \in \S^ V} \|\sv{}{ V}\|_{\b{}} < \infty\) (recall that \(\|\sv{}{ V}\|_{\b{}} = \sum_{\v \in  V} \b{\v} |\sv{\v}{ V}|\) where \(\b{\v} = \b{}^{-\numb}\) for some \(\b{} > \degr\) if \(\v \in  V\sln{\numb}\setminus  V\sln{\numb-1}\)).

\ind Now fix \(\ep > 0\). Let \(M\) be large enough that

\[\sup_{\sv{}{\left( V\sln{M}\right)^c} \in \S^{\left( V\sln{M}\right)^c}} \left\|\sv{}{\left( V\sln{M}\right)^c}\right\|_{\b{}} \leq |\S|\sum_{\v \notin  V\sln{M}} \b{\v} < \ep/4.\]

Recall from Definition \ref{sec::TL:Def::modulus}, the c\`adl\`ag version of the modulus of continuity is given by,

\[\omega'_{\X{}{}}(\delta) = \inf_{\{\t\sln{\it}\}} \sup_\it \sup_{\tt,\ttt \in [\t\sln{\it-1},\t\sln{\it})} \|\X{}{\tt} - \X{}{\ttt}\|_{\b{}},\]

where \(\{\t\sln{\it}\} := 0 \leq \t\sln{1} < \t\sln{2} < \cdots < \t\sln{\cdot} = \T\) and \(\min_\it|\t\sln{\it} - \t\sln{\it-1}| > \delta\). Note that \(\X{}{}{\numb}\) is \(\Omega{ V\sln{\numb}}{\T}\)-valued and can be embedded in \(\Omega{ V}{\T}\) the same way we embed \(\S^{ V\sln{\numb}}\) in \(\S^ V\): let \(\X{\v}{}{\numb} \equiv 0\) for \(\v \notin  V\sln{\numb}\). Recall also that \(\proj{\U}{\T}\) is the projection to \(\Omega{\U}{\T}\) for all \(\U\subseteq  V\). Then for any \(\numb\in \mb{N}\),

\begin{align*}
\pr\left(\sup_\numb \omega'_{\X{}{}{\numb}}(\delta) > \epsilon\right) &= \pr\left(\sup_\numb\inf_{\{\t\sln{\it}\}}\sup_\it\sup_{\tt,\ttt \in [\t\sln{\it-1},\t\sln{\it})} \sum_{\v \in  V} \b{\v}|(\X{\v}{\tt}{\numb} - \X{\v}{\ttt}{\numb}| > \epsilon \right)\\
&\leq \pr\left(\sup_\numb \left(\omega'_{\proj{ V\sln{M}}{\T}(\X{}{}{\numb})}(\delta) + \omega'_{\proj{\left( V\sln{M}\right)^c}{\T}(\X{}{}{\numb})}(\delta)\right) > \ep\right)\\
&\leq \pr\left(\sup_\numb \omega'_{\proj{ V\sln{M}}{\T}(\X{}{}{\numb})}(\delta) > \ep/2\right) \os{\delta\searrow 0}{\ra} 0.
\end{align*}

The last inequality arises because

\[\omega'_{\proj{\left( V\sln{M}\right)^c}{\T}(\X{}{}{\numb})}(\delta) \leq 2 \sup_{\t\in [0,\T]} \left\|\proj{\left( V\sln{M}\right)^c}{\T}(\X{}{}{\numb})\right\|_{\b{}} \leq 2 \sup_{\sv{}{\left( V\sln{M}\right)^c} \in \S^{\left( V\sln{M}\right)^c}} \left\|\sv{}{\left( V\sln{M}\right)^c}\right\|_{\b{}} < \ep/2\]
...
&= \left|\sum_{\v \in  V\sln{\numb}} \sum_{\s \in \S} \IGr{\v}(\proj{ V\sln{\numb}}{}(\sv{}{ V}),\s)[f(\sv{}{ V}+\s\ev{\v}) - f(\sv{}{ V})] - \sum_{\v \in  V}\sum_{\s \in \S} \IGr{\v}(\sv{}{ V},\s)[f(\sv{}{ V}+\s\ev{\v}) - f(\sv{}{ V})]\right|\\
&\leq  \left|\sum_{\v \in  V\sln{\numb}} \sum_{\s \in \S} \IGr{\v}(\proj{ V\sln{\numb}}{}(\sv{}{ V}),\s)[f(\sv{}{ V}+\s\ev{\v}) - f(\sv{}{ V})] - \sum_{\v \in  V\sln{\numb}}\sum_{\s \in \S} \IGr{\v}(\sv{}{ V},\s)[f(\sv{}{ V}+\s\ev{\v}) - f(\sv{}{ V})]\right|\\
&\ind  +\left|\sum_{\v \notin  V\sln{\numb}} \sum_{\s\in \S} \IGr{\v}(\sv{}{ V},\s)[f(\sv{}{ V}+\s\ev{\v}) - f(\sv{}{ V})]\right|\\
&\leq \sum_{\v \in  V\sln{\numb}}\sum_{\s \in \S} \left|\IGr{\v}(\proj{ V\sln{\numb}}{}(\sv{}{ V}),\s) - \IGr{\v}(\sv{}{ V},\s)\right|\triangle_f(\v) + \sum_{\v \in  V\setminus  V\sln{\numb}} \const{}\triangle_f(\v)\\
&\os{Assump. \ref{subsec::Assum:Assu::CI}.2}{=} \sum_{\v \in  V\sln{\numb}\setminus V\sln{\numb-1}} |\S|\const{} \triangle_f(\v) + \sum_{\v \in  V\setminus  V\sln{\numb}} \const{}\triangle_f(\v)\\
&\leq |\S|C\sum_{\v \in  V\setminus  V\sln{\numb-1}} \triangle_f(\v) \os{n\ra\infty}{\ra} 0.
\end{align*}
...
\[\typset = \bigcup_{\it = 1}^\numb [\tt^\it,\t^\it)\times \typset\sln{\it},\]

where \(\{[\tt^\it,\t^\it):\it=1,\dots,\numb\}\) are disjoint, if \(\tt^\it = \t^\it = 0\), then \([\tt^\it,\t^\it) = \{0\}\) and \(\typset\sln{\it}\) are all \(\sigma(\crate{\v}{\t}(\s):\s \in \S,\t\in [0,\tt^\it])\)-measurable. 
...
\[\left(\proj{\tree\sln{1}}{\T}(\X{}{})\right)^\t = \left(\proj{\tree\sln{1}}{\T}(\X{}{})\right)^0 + \sum_{\v \in \tree\sln{1}}\ev{\v}\int_\S\int_0^\t \s\,\poiss{\v}\left(d\tt,\left(0,\rate{\v}(\X{}{\tt-},\s)\right],d\s\right).\]
...
Therefore it suffices to derive the \(\F{\tree\sln{1}}{\t}\)-intensity of \(\rp{}\). \(\rp{}\) has \(\F{ V}{\t}\)-intensity \(\rate{\rp{}}(\X{}{\t-},\mark{}) := \sum_{\v \in \tree^1}\sum_{\s\in S}\mb{I}_{\mark{} = \s\ev{\v}}\rate{\v}(\X{}{\t-},\s)\). Each \(\rate{\v}\) is clearly left-continuous in time, so \(\rate{\rp{}}\) is also left-continuous in time. Recall that \(\m{}{}{}\) is the law of \(\X{}{}\). Then,

\[\exmu{\m{}{}{}}{\rate{\rp{}}(\X{}{\t-},\mark{})|\F{\tree\sln{1}}{\t-}} = \sum_{\v \in \tree\sln{1}}\sum_{\s\in S} \mb{I}_{\mark{} = \s\ev{\v}}\exmu{\m{}{}{}}{\rate{\v}(\X{}{\t-},\s)|\F{\tree\sln{1}}{\t-}}.\]

Applying Theorem \ref{sec::Main:Thm::CI} and symmetry of \(\m{}{}{}\) yields,

\begin{align*}
\exmu{\m{}{}{}}{\rate{\v}(\X{}{\t-},\s)|\F{\tree\sln{1}}{\t-}} &= \exmu{\m{}{}{}}{\rate{\v}(\X{}{\t-},\s)|\X{\tree\sln{1}}{[0,\t)}}\\
&\os{\te{Thm \ref{sec::Main:Thm::CI}}}{=} \exmu{\m{}{}{}}{\rate{}(\X{\cl{\v}}{\t-},\s)|\X{\{\v,\root\}}{[0,\t)}} \\
&\os{\te{Sym}}{=} \exmu{\rxvtts{}{}\sim \m{}{}{}}{\rate{}(\rxvtt{\tree\sln{1}}{\t-},\s)|\rxvtts{\{\root,1\}}{[0,\t)} = \X{\{\v,\root\}}{[0,\t)}}\\
&= \exmu{\rxvtts{}{}\sim \proj{\tree\sln{1}}{\T}(\m{}{}{})}{\rate{}(\rxvtt{\tree\sln{1}}{\t-},\s)|\rxvtts{\{\root,1\}}{[0,\t)} = \X{\{\v,\root\}}{[0,\t)}}\\
&= \exmu{\cm_\t\left(\X{\{\v,\root\}}{[0,\t)},\proj{\tree\sln{1}}{\T}(\m{}{}{})\right)}{\rate{}(\X{\tree\sln{1}}{\t-},\s)}\\
...
Let \(\mm{}{}{} = \law(\rxvtts{}{})\). Suppose \(\mmm{\v}{0,}{1}\ll\mm{}{0}{}\) for all \(\v\in\tree\sln{1}\). Let \(\{\mm{\v}{}{}:\v\in V\}\) be the distributions of a sequence of i.i.d. \(\S^{\v}\)-valued c\`adl\`ag processes. For any \(\U\subseteq  V\), let 
...
\begin{lem}
For any \(\v \in  V\setminus\{\root\}\), let \(\p{\v}\) be the parent node of \(\v\) (neighboring node of \(\v\) closest to \(\root\)). Fix \(\numb \in \mb{N}\). Let \(\mmm{}{}{\numb} \in \pmsr\left(\Omega{\tree\sln{\numb}}{\T-}\right)\) be the law of the unique solution to the following SDE:

\begin{equation}
\X{\v}{\t} = \begin{cases}
\X{\v}{0} + \int_{(0,\t]}\int_\S\int_{\left(0,\crate{}{}(\X{\{\v,\p{\v}\}}{[0,\tt)},\s)\right]}\s\,\poiss{\v}(d\r,d\s,d\tt)\te{ if } \v \in \tree\sln{\numb}\setminus \tree\sln{\numb-1}\\
\X{\v}{0} + \int_{(0,\t]}\int_\S\int_{\left(0,\rate{}(\X{\cl{\v}}{\tt-},\s\right)}\s\,\poiss{\v}(d\r,d\s,d\tt)\te{ if } \v \in \tree\sln{\numb-1}
\end{cases}
\label{subsec::ProofU:eqn::extendedSDE}
\end{equation}
\labe{subsec::ProofU:eqn::extendedSDE}

Then for any \(\alt{\numb} < \numb\), \(\mmm{}{\t}{\alt{\numb}} = \proj{\tree\sln{\alt{\numb}}}{\t}(\mmm{}{\t}{\numb})\).
...
\[\dense{\numb}{\t}(\X{}{}{\numb}) := \dense{\numb-1}{\t}(\X{\tree\sln{\numb-1}}{}{\numb})\prod_{\v\in \tree\sln{\numb-1}\setminus\tree\sln{\numb-2}} \cdense{}{\t}(\X{\c{\v}}{}{\numb};\X{\{\v,\root\}}{}{\numb})\]

for \(\numb > 1\) (\(\tree\sln{0} = \{\root\}\)). We can show using induction,
...
\begin{equation}
\dense{\numb}{\t}(\X{}{}{\numb}) = \frac{d\mmm{}{\t}{\numb}}{d\mm{}{0}{\numb}}\exp\left(\sum_{\v\in\tree\sln{\numb-1}}\ds{\v}{\t}(\X{\cl{\v}}{}{\numb},\rate{}) + \sum_{\v\in\tree\sln{\numb}\setminus\tree\sln{\numb-1}} \ds{\v}{\t}(\X{\{v,\p{\v}\}}{}{\numb},\crate{}{})\right).
...
&=\left(\frac{d\mmm{}{0}{\numb-1}}{d\mm{}{0}{\numb-1}}\prod_{\v\in\tree\sln{\numb-1}\setminus\tree\sln{\numb-2}}\frac{d\mmm{\c{\v}}{0,}{}}{d\mm{\c{\v}}{0,}{}}\right)\exp\Bigg(\sum_{\v\in\tree\sln{\numb-2}}\ds{\v}{\t}(\X{\cl{\v}}{}{\numb},\rate{}) + \sum_{\v\in\tree\sln{\numb-1}\setminus\tree\sln{\numb-2}} \ds{\v}{\t}(\X{\{\v,\p{\v}\}}{}{\numb},\crate{}{})\\
&\ind + \sum_{\v\in\tree\sln{\numb-1}\setminus\tree\sln{\numb-2}}\left(\sum_{\vv\in \c{\v}} \ds{\vv}{\t}(\X{\{\vv,\v\}}{}{\numb},\crate{}{}) + \ds{\v}{\t}(\X{\cl{\v}}{}{\numb},\rate{}) - \ds{\v}{\t}(\X{\{\v,\p{\v}\}}{}{\numb},\crate{}{})\right)\Bigg)\\
&=\frac{d\mmm{}{0}{\numb}}{d\mm{}{0}{\numb}}\exp\left(\sum_{\v\in\tree\sln{\numb-1}}\ds{\v}{\t}(\X{\cl{\v}}{}{\numb},\rate{}) + \sum_{\v\in\tree\sln{\numb}\setminus\tree\sln{\numb-1}} \ds{\v}{\t}(\X{\{\v,\p{\v}\}}{}{\numb},\crate{}{})\right).
\end{align*}
...
This proves that \(\proj{\tree\sln{\numb-1}}{\t}(\mmm{}{\t}{\numb}) = \mmm{}{\t}{\numb-1}\). Furthermore, for any \(\alt{\numb} < \numb\),

\[\proj{\tree\sln{\alt{\numb}}}{\t}(\mmm{}{\t}{\numb}) = \proj{\tree\sln{\alt\numb}}{\t}\circ\proj{\tree\sln{\alt{\numb} + 1}}{\t} \circ\cdots\circ \proj{\tree\sln{\numb-1}}{\t}(\mmm{}{\t}{\numb}) = \proj{\tree\sln{\alt\numb}}{\t}\circ\proj{\tree\sln{\alt{\numb} + 1}}{\t} \circ\cdots\circ \proj{\tree\sln{\numb-2}}{\t}(\mmm{}{\t}{\numb-1}) = \cdots = \mmm{}{\t}{\alt{\numb}}.\]
...
\[\rate{\numb}(\t,\sv{\tree\sln{\numb}}) = \begin{cases}
\rate{}(\X{\cl{\v}}{\t-}{\numb},\s) &\te{ if } \sv{\tree\sln{\numb}} = \s\ev{\v},\s \in \S\setminus\{0\},\v\in\tree\sln{\numb-1}\\
\crate{}{}(\X{\{\v,\p{\v}\}}{[0,\t)}{\numb},\s) &\te{ if } \sv{\tree\sln{\numb}} = \s\ev{\v},\s\in\S\setminus\{0\},\v\in\tree\sln{\numb}\setminus\tree\sln{\numb-1}\\
0 &\te{ otherwise.}
\end{cases}\]
...
\begin{equation}
\poiss{\numb}(\evnt) = \alt{\poiss{\numb}}\left(\evnt\cap\{(t,\sv{\tree\sln{\numb}},\r):\r > \rate{\numb}(\t,\sv{\tree\sln{\numb}})\}\right) + \#\{(\rt{\numb,\it}_\v,\mark{\numb,\it}_\v,\rv_{\v}^{\it}\rate{\numb}(\rt{\numb,\it}_\v,\mark{\numb,\it}_\v) \in \evnt: \v\in \tree\sln{\numb}\}.
...
\[\IG\sln{\numb}f(\sv{}{ V}) = \sum_{\v\in  V}\sum_{\s \in \S} \IGr{\v}\sln{\numb}(\sv{}{ V},\s)[f(\sv{}{ V} + \s\ev{\v}) - f(\sv{}{ V})].\]

If there exists some \(\const{} < \infty\) such that

\[\sup_{\numb}\sup_{\sv{}{ V}\in \S^ V}\sup_{\v\in V}\sup_{\s\in \S} \IGr{\v}\sln{\numb}(\sv{}{ V},\s) \leq \const{},\]
...
\begin{align*}
\sup_\it\sup_{\tt,\ttt \in [\t\sln{\it-1},\t\sln{\it})} \sum_{\v\in \U} \b{\v}|\X{\v}{\tt}{\numb} - \X{\v}{\ttt}{\numb}|\leq \sup_\it\sup_{\tt,\ttt \in [\t\sln{\it-1},\t\sln{\it})} \sum_{\v\in \U} \b{\v}|\rxvtt{\v}{\tt} - \rxvtt{\v}{\ttt}|.
\end{align*}

Fix \(\delta > 0\). Taking the infimum over all partitions with minimum interval length \(\delta\) yields,

\begin{align*}
\omega'_{\proj{\U}{\T}\left(\X{}{}{\numb}\right)}(\delta) &= \inf_{\{\t\sln{\it}\}}\sup_\it\sup_{\tt,\ttt \in [\t\sln{\it-1},\t\sln{\it})} \sum_{\v\in \U} \b{\v}|\X{\v}{\tt}{\numb} - \X{\v}{\ttt}{\numb}|\\
...
\begin{align*}
\pr\left(\poiss{\numb}\left(\inte\times R\times \alt{\inte}\right) = 0\right) &= \sum_{\sv{}{\tree\sln{\numb}} \in \alt{S}^\numb} \pr\left(\poiss{\numb}^{\sv{}{\tree\sln{\numb}}}\left(\inte\times R\times \alt{\inte}\right) = 0\right)\pr(\X{}{0}{\numb} = \sv{}{\tree\sln{\numb}})\\
&= \exp\left(-|R| |\inte||\alt{\inte}|\right)\sum_{\sv{}{\tree\sln{\numb}} \in \alt{S^\numb}} \pr(\X{}{0}{\numb} = \sv{}{\tree\sln{\numb}})\\
