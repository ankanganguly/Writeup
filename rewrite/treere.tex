...
\newcommand{\const}{C}							%arbitrary constant
...
%
%\begin{prop}
%Let \((G,\Xf\gind{G})\) satisfy assumptions \ref{a::bddinit} and \ref{a::liprx}, and suppose \(\phi\) is a symmetry of \(\Xf\gind{G}\). Then,
%
...
%\end{prop}
%\begin{proof}
%By theorem \ref{wp::wp}, \(\Xf\gind{G}\) is the unique strong solution to the equation,
...
%
%\begin{assu}
%Let \((G,\Xf\gind{G}) \in \Gs\sp{\cad}\) be well-defined. Then all automorphisms of assumption \ref{Main::admissible} are symmetries of \((G,\Xf\gind{G})\).
%\label{Main::padmin}
%\end{assu}
%
%\begin{thms}
%Let \((G,\Xf) \in \Gs\sp{\cad}\) satisfy assumptions \ref{a::bddinit},\ref{a::bddr}, \ref{a::liprx}, \ref{CI::indinit}, \ref{Main::admissible} and \ref{Main::padmin}. Let \(A\) be the admissible set and \(\{B_i\}\) the branches of \(A\). Let \(\inte{A} = \{v \in A: \neigh{v} \subseteq A\}\). Consider the following equations:
%
%\begin{align}
%\Xg\vind{v}\tme{t} &= 
%\begin{cases}
